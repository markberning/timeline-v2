...
3. **Single-civ TEST = carthage (commit `c112f53`, pushed, NOT deployed — review checkpoint).** Full recipe: 8 per-chapter agents authored web-verified description + Explore-further beat for all 74 surfaced events + ran the photo vision pass (52 commonsFile overrides viewed+captioned, 22 honest rejections). Gates GREEN: G14 74/74, fix-links --strict clean. Review at **localhost:3000/civ/carthage** (dev server running).
   **KEY SWEEP LESSON — cross-chapter photo dedup is mandatory.** Per-chapter agents working in isolation independently grab the SAME iconic images for related events across chapters (Capua Hannibal bust, Byrsa Punic houses, Barcid silver coins, votive stelae) → fix-links dedup drops the losers → 3 resolver rounds needed. The sweep recipe must add a coordinator dedup pass (the holistic `byFile` duplicate-finder) BEFORE running gates. Also: un-rejecting an event that was in the corpus-wide `.image-rejections.json` from its original build requires REMOVING it there too, or the new override image is suppressed (line 412 of enrich-events drops events with no cached thumbnail). 2 borderline-anachronistic picks flagged for the user (Slodtz 18th-c. Hannibal statue; Roman Antonine Baths for the 146 BCE destruction).
4. **NEXT: corpus-wide event-upgrade sweep — 104 civs (103 NONE + celtic 12/67 partial), ~8k surfaced events.** Awaiting user green-light after carthage review. Recipe proven on carthage; batch-deploy every 2–4 civs per feedback_sweep_autonomy. The G14 baseline shrinks as each civ is swept to 100%.

**🆕🆕 (2026-05-29 PM — WAVE 2 SHIPPED + DEPLOYED, event-popup standard LOCKED):**
**The 3 `atlantic-world` civs are LIVE on prod** (Version `cf6f5302`, pushed `1393d45`, verified 200 at `/civ/{id}`): **age-of-exploration** (14ch), **atlantic-slave-trade** (11ch), **latin-american-independence** (11ch) — new `atlantic-world` chain (aoe→ast→lai). User lifted the deploy hold ("deploy all 6 civs when this batch completed") so **wave-1's 3 (medieval-japan/islamic-persia/muscovite-russia) deployed in the same build** — all 6 now live. Full pipeline ran: pools→narratives→4-persona audit (0 WEAK/REWRITE; Persona-B web-fact-checked, ~all stats confirmed, 1 real fix: Toussaint's prison = Jura not Alps)→born-verified links (all gates 0-ERROR, G10/11/12 0-FAIL)→summaries→intros (G13 0-ERROR + fact/voice critic)→36 maps→3 emblems→backward cross-cultural pass (52 cross-links into 12 neighbors)→ship-check CLEAR→deploy.
**Maps:** ran into Gemini API contention when 3 builds ran concurrently (transient "fetch failed" QA errors inflated attempts) → fix = run **serial, one civ at a time**. aoe ch3/ch6 needed prompt rewrites (ch6 prompt was self-contradictory); ch14 was a **QA false-positive** (correct map flagged) → waived in `content/.map-waivers-age-of-exploration.json`.
**🔑 NEW LOCKED RULES baked into pipeline this session:**
1. **Maps = 3-attempt cap** (one `maps-build` run = the budget; after that, note `ch N: reason` to user + MOVE ON — never grind). **No blind trust of map QA** — eyeball flagged maps, waive false-positives. (CLAUDE step 12 + `map-prompts/README.md` + `memory/feedback_map_three_attempt_cap`.)
2. **Event-popup standard** (`celtic-cultures` ch1 = user-approved reference pilot): a **two-part "explore further" card** — `description` (tight house-voice define) + `details:[{label:"Explore further",text}]` (born-verified info beyond the narrative); **vision-verified photo** (LOOK at it; set as **`commonsFile` in reference-data** — the lever `fix-links --strict` reads, NOT `.image-overrides.json`) + **English house-voice caption** (`.caption-overrides.json`). EventSheet UI redesigned (Piece A shipped: house-voice lead, photo inset under title, raw wiki-extract dropped). Baked: CLAUDE **step 0** (writing) + **step 13** (photo/caption VISION pass) + `memory/project_event_link_enrichment` + backlog #20. **So the remaining 8 civs build popups+photos correctly from the start.**
**▶ TWO OPEN PROGRAMS (user's call which first):** (a) **remaining 8 civs** (#39–46: ethiopian-empire, great-zimbabwe, kingdom-of-kongo, benin-and-ife, mainland-southeast-asia, tibetan-empire, haudenosaunee, the-franks) — now inherit the popup standard; (b) **corpus-wide event-popup rollout** (legacy 100 + wave-1 + wave-2 retro): triage (`fix-links` photo flags) → vision-verify → 2-part copy where thin. GREENLIT, not started; whole-corpus vs worst-first TBC. **Wave-1+2 shipped with vision-verified PHOTOS but single-blurb writing — their "explore further" beat comes in the rollout.**

**(prior) 🆕 NEWEST (2026-05-29 ~01:30 PDT — the-17 NEW-CIV build, WAVE 1 of 4 COMPLETE, NOT DEPLOYED):**
...
**#19 INTRO RETROFIT — ✅ COMPLETE (2026-05-28).** 102/102 done, intro-baseline 102->0 (B1-B21 all committed+pushed; B21 modern-China = chinese-revolution,rise-of-china). EVERY civ in the corpus now carries gated chapter-intro cards; `audits/intro-baseline.json` is empty, so all 102 are zero-tolerance on G13 going forward. Per-civ gate was 3-stage: lint-intros --strict 0-ERROR + a combined fact+voice critic agent (every civ graded VOICE STRONG; the critics caught ~40 real accuracy slips corpus-wide, each fixed before commit — e.g. David=Saul's son-in-law, Slater 1789, Cateau-Cambrésis didn't end all Italian independence, Holodomor-genocide kept disputed, Tiananmen toll kept a range). **NOT YET DEPLOYED — user is holding the deploy; intros are in git + parsed into content JSON but a `rm -rf out && npm run build && npx wrangler deploy` is still owed once the user gives the go-ahead.** Batches committed+pushed (NOT deployed, user holding): B1 Northern-European (celtic,germanic-tribes,the-goths,migration-period,anglo-saxon-england), B2 Greco-Roman (minoan,mycenaean,ancient-greece,ancient-rome,byzantine-empire), B3 early-Chinese (ancient-china,shang,zhou,qin,han), B4 Nile (early-dynastic/old-kingdom/new-kingdom/late egypt, ancient-nubia), B5 Indian (indus-valley,vedic-period,maurya,post-maurya-kingdoms,gupta). Pipeline per civ: author agent -> lint-intros --strict -> ONE combined fact+voice critic agent -> apply fixes -> debaseline (remove from audits/intro-baseline.json) -> scoped parse -> commit. 5/wave. Authoritative progress = audits/intro-baseline.json (civs still listed = not yet done). early-medieval-europe = template.
