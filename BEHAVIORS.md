...
### Data
- 71 TLs in `NAVIGATOR_TLS` (`src/lib/navigator-tls.ts`): id, label, subtitle, region, startYear, endYear, optional `isReal` and `hasContent` flags. 39 TLs have `hasContent: true` (full list in `navigator-tls.ts`); the remaining 52 render at opacity 0.35.
...
- **Current curation state (post 2026-04-24 cleanup)**: all 39 shipped TLs have cross-link files (~550+ total cross-links). The 2026-04-24 pass added cross-links for the 3 standalone TLs (phoenicia 18, ancient-israel 15, polynesian-voyagers 17), fixed 26 broken matchTexts across 10 TLs, and brought parse warnings to zero. Adding a new TL should include cross-links as part of the standard pipeline.
