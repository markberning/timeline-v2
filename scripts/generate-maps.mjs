...
// It ran up a $483 Gemini bill on the May civ-map build. Maps now come from real /
// period public-domain sources, never generated. See audits/map-generation-disabled.md.
// (--dry-run is allowed: it makes no API call.) Deliberate one-off override: ALLOW_GEMINI_MAP_GEN=1.
if (!dryRun && process.env.ALLOW_GEMINI_MAP_GEN !== '1') {
  console.error('DISABLED: Gemini map generation is retired (cost control).')
...
function preprocessPrompt(prompt) {
  ...
  const globalRules = [
    '**LABEL RULE:** Spell every label correctly and place each clearly. A long river MAY carry its label more than once along its course (normal cartographic practice) — that is fine as long as each instance is spelled correctly and the repeats are well separated (not touching, stacked, or adjacent). Do NOT stamp the SAME city/site/region label twice, do NOT place two copies of any label next to each other, and do NOT repeat an annotation or caption anywhere on the map.',
    '',
    ...
    '',
    '**LABELS COME ONLY FROM THE BULLETS — NEVER FROM PROSE:** The ONLY text you may render anywhere on the map is: the title bar, the bare proper-name labels listed in the site/region bullets, and the single annotation explicitly specified. Every other sentence in this prompt — the opening "Map of …" scene description, any placement guidance, any clause after an em-dash or inside parentheses in a bullet — is INSTRUCTION TO YOU describing what to draw and where, NOT text to print. Never lift a word, phrase, date, or parenthetical from a scene sentence or a description onto a label (on a prior run the scene word "amputated" was stamped onto a site as "(amputated territory)" — never do this). A label is the bare proper name and nothing else appended.',
    '',
