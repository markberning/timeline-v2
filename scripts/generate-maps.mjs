...
function preprocessPrompt(prompt) {
  ...
  const globalRules = [
    '**DUPLICATE-LABEL RULE (the #1 corpus defect):** Draw every city, region, sea, and river label EXACTLY ONCE. Long rivers are the most frequent failure: a long river (for example the Nile, Yellow River, Niger, Volga, Euphrates, Tigris, Ganges, Danube, Mississippi) gets ONE label along its course and never a second copy near another bend. A long river that crosses the entire map still gets exactly ONE label total — not one per segment, bend, or side. Tigris, Euphrates, Orontes, Yellow River, Yangtze: one label each, no exceptions. Never repeat any annotation text anywhere.',
    '',
    ...
    '',
    '**NO MODERN POLITICAL GEOGRAPHY:** This is a historical map. Do not draw modern country names (for example PAKISTAN, IRAN, TURKEY, INDIA, JAPAN, CHINA, KOREA, EGYPT, MEXICO) and do NOT draw ANY boundary, border, national, administrative, prefecture, province, or internal-division lines anywhere (solid, dotted, or dashed). The only lines permitted on the map are coastlines, the rivers named in this prompt, and the single annotation arrow or shaded zone. On wide-area, continental, or world maps especially: the basemap is blank land plus coastlines plus the prompt rivers ONLY — zero internal division lines and zero country names (never label "Belize", "Nepal", "Pakistan", or any modern nation). Show only the regions, sites, and labels named in this prompt.',
    '',
