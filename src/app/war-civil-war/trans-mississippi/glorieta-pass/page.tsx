...
const CRUMBS = civilWarCrumbs({ theatre: 'tmis', battleId: 't-glorieta' })

const HERO_IMG = '/war-img/glorieta-pass-hero.jpg' // National Guard Heritage painting (PD)
...
const SECTION_IMG: Record<string, string> = {
  ...
  'the-wagons': '/war-img/cmdr/chivington.jpg',
}
const sectionHref = (id: string) => `/war-civil-war/trans-mississippi/glorieta-pass/s/${id}`
  ...
  return (
    <div style={{ position: 'relative', height: 260, overflow: 'hidden' }}>
      {failed
        ...
        : <img src={HERO_IMG} alt="" onError={() => setFailed(true)} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 40%' }} />}
      ...
      <div style={{ position: 'absolute', left: 16, right: 16, bottom: 14 }}>
        ...
        <h1 style={{ margin: '6px 0 0', fontFamily: SERIF, fontWeight: 500, fontSize: 30, lineHeight: 1.05, letterSpacing: -0.5, color: '#fff' }}>Battle of Glorieta Pass</h1>
        <div style={{ fontFamily: SERIF, fontSize: 14, color: 'rgba(255,255,255,0.82)', marginTop: 4 }}>March 26–28, 1862 · New Mexico Territory</div>
      </div>
    </div>
  )
}
      ...
      {open && (
        <div style={{ marginTop: 14 }}>
          <div style={{ display: 'flex', marginBottom: 18 }}>
            {[['Duration', '3 days'], ['Casualties', '~270'], ['Winner', 'Union (strategic)']].map(([k, v], i) => (
