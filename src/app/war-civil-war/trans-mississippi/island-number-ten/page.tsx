...
const CRUMBS = civilWarCrumbs({ theatre: 'tmis', battleId: 't-island10' })
...
const TL_META: Record<string, { size: CardSize; date: string; palette: [string, string, string] }> = {
  'the-gibraltar': { size: 'm', date: 'Mar', palette: ['#2a3a44', '#22303a', '#0a0e10'] },
  'the-canal': { size: 'l', date: 'Apr', palette: ['#3a3320', '#2a241a', '#0a0806'] },
  'the-trap': { size: 'xl', date: 'Apr 8', palette: ['#7a4a18', '#3a2a14', '#0e0905'] },
}
const SECTION_IMG: Record<string, string> = {
  'the-gibraltar': '/war-img/island-number-ten-strategy.png',
  'the-canal': '/war-img/island-number-ten.png',
  'the-trap': '/war-img/island-number-ten-trap.png',
}
const sectionHref = (id: string) => `/war-civil-war/trans-mississippi/island-number-ten/s/${id}`
  ...
  return (
    <div style={{ position: 'relative', height: 260, overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', left: 16, right: 16, bottom: 14 }}>
        ...
        <h1 style={{ margin: '6px 0 0', fontFamily: SERIF, fontWeight: 500, fontSize: 30, lineHeight: 1.05, letterSpacing: -0.5, color: '#fff' }}>Battle of Island Number Ten</h1>
        <div style={{ fontFamily: SERIF, fontSize: 14, color: 'rgba(255,255,255,0.82)', marginTop: 4 }}>Feb 28 – Apr 8, 1862 · Mississippi River</div>
      </div>
    </div>
  )
}
      ...
      {open && (
        <div style={{ marginTop: 14 }}>
          <div style={{ display: 'flex', marginBottom: 18 }}>
            {([['Duration', '~6 weeks'], ['Captured', '~4,500'], ['Winner', 'Union']] as const).map(([k, v], i) => (
        ...
        {view === 'dossier' ? (
          ...
          <div style={{ padding: '8px 0 20px' }}>
            <p style={{ fontFamily: SERIF, fontSize: 16, lineHeight: 1.6, margin: '8px 16px 4px', color: 'color-mix(in srgb, var(--foreground) 78%, transparent)' }}>
              The river fortress that fell to a shovel and a thunderstorm — almost without blood.
            </p>
            <CordTimeline>
              {SECTIONS.map(s => {
