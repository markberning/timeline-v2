...
const CRUMBS = civilWarCrumbs({ theatre: 'west', battleId: 'w-lookout' })
...
const TL_META: Record<string, { size: CardSize; date: string; palette: [string, string, string] }> = {
  'the-army-under-the-mountain': { size: 'l', date: 'Setup', palette: ['#3a4a2a', '#283420', '#0a0e08'] },
  'into-the-fog': { size: 'xl', date: 'Nov 24', palette: ['#1d3a5a', '#16283a', '#080c12'] },
  'the-battle-above-the-clouds': { size: 'l', date: 'Afternoon', palette: ['#3a4150', '#28303a', '#080c12'] },
  'the-flag-at-dawn': { size: 'm', date: 'Nov 25', palette: ['#3a2e21', '#2a221c', '#0a0806'] },
}
const SECTION_IMG: Record<string, string> = {
  'the-army-under-the-mountain': '/war-img/lookout-mountain-overview.png',
  'into-the-fog': '/war-img/lookout-mountain.png',
  'the-battle-above-the-clouds': '/war-img/cmdr/hooker.jpg',
  'the-flag-at-dawn': '/war-img/cmdr/grant.jpg',
}
const sectionHref = (id: string) => `/war-civil-war/western/lookout-mountain/s/${id}`
...
function HeroImg() {
  ...
  return (
    <div style={{ position: 'relative', height: 260, overflow: 'hidden' }}>
      {failed
        ? <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, #3a4150, #28303a 55%, #0a0806)' }} />
      ...
      <div style={{ position: 'absolute', left: 16, right: 16, bottom: 14 }}>
        ...
        <h1 style={{ margin: '6px 0 0', fontFamily: SERIF, fontWeight: 500, fontSize: 30, lineHeight: 1.05, letterSpacing: -0.5, color: '#fff' }}>Battle of Lookout Mountain</h1>
        <div style={{ fontFamily: SERIF, fontSize: 14, color: 'rgba(255,255,255,0.82)', marginTop: 4 }}>November 24, 1863 · Chattanooga, Tennessee</div>
      </div>
    </div>
  )
}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 16, fontFamily: SANS, fontSize: 11, color: 'color-mix(in srgb, var(--foreground) 65%, transparent)' }}>
        ...
        <span><span style={{ color: ACCENTS.rust }}>■</span> Confederacy ~{num(CAS.csa)} (~1,064 captured)</span>
      </div>
    </div>
  )
}
      ...
      {open && (
        <div style={{ marginTop: 14 }}>
          <div style={{ display: 'flex', marginBottom: 18 }}>
            {([['Duration', '1 day'], ['Casualties', '~1,920'], ['Winner', 'Union']] as const).map(([k, v], i) => (
        ...
        {view === 'dossier' ? (
          ...
          <div style={{ padding: '8px 0 20px' }}>
            <p style={{ fontFamily: SERIF, fontSize: 16, lineHeight: 1.6, margin: '8px 16px 4px', color: 'color-mix(in srgb, var(--foreground) 78%, transparent)' }}>
              “The Battle Above the Clouds” — a foggy mountain, a watching army, and a legend several sizes too big for the modest fight that earned it.
            </p>
            <CordTimeline>
              {SECTIONS.map(s => {
