...
// BATTLE dossier (Second Battle of Corinth). Same shape as Shiloh: hero · collapsible At-a-glance
...
const CRUMBS = civilWarCrumbs({ theatre: 'west', battleId: 'w-corinth' })
...
const TL_META: Record<string, { size: CardSize; date: string; palette: [string, string, string] }> = {
  'the-crossroads': { size: 'l', date: 'Setup', palette: ['#3a4a2a', '#283420', '#0a0e08'] },
  'october-third': { size: 'l', date: 'Oct 3', palette: ['#1d3a5a', '#16283a', '#080c12'] },
  'battery-robinett': { size: 'xl', date: 'Oct 4', palette: ['#7a1422', '#3a1208', '#0a0606'] },
  'what-it-protected': { size: 'm', date: 'After', palette: ['#3a2e21', '#2a221c', '#0a0806'] },
}
const SECTION_IMG: Record<string, string> = {
  'the-crossroads': '/war-img/cmdr/van-dorn.jpg',
  'october-third': '/war-img/corinth-overview.png',
  'battery-robinett': '/war-img/corinth.png',
  'what-it-protected': '/war-img/cmdr/rosecrans.jpg',
}
const sectionHref = (id: string) => `/war-civil-war/western/corinth/s/${id}`
  ...
  return (
    <div style={{ position: 'relative', height: 260, overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', left: 16, right: 16, bottom: 14 }}>
        ...
        <h1 style={{ margin: '6px 0 0', fontFamily: SERIF, fontWeight: 500, fontSize: 30, lineHeight: 1.05, letterSpacing: -0.5, color: '#fff' }}>Second Battle of Corinth</h1>
        <div style={{ fontFamily: SERIF, fontSize: 14, color: 'rgba(255,255,255,0.82)', marginTop: 4 }}>October 3–4, 1862 · Corinth, Mississippi</div>
      </div>
    </div>
  )
}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 16, fontFamily: SANS, fontSize: 11, color: 'color-mix(in srgb, var(--foreground) 65%, transparent)' }}>
        ...
        <span><span style={{ color: ACCENTS.rust }}>■</span> Confederacy ~4,200–4,800</span>
      </div>
    </div>
  )
}
      ...
      {open && (
        <div style={{ marginTop: 14 }}>
          <div style={{ display: 'flex', marginBottom: 18 }}>
            {([['Duration', '2 days'], ['Casualties', '~7,000'], ['Winner', 'Union']] as const).map(([k, v], i) => (
        ...
        {view === 'dossier' ? (
          ...
          <div style={{ padding: '8px 0 20px' }}>
            <p style={{ fontFamily: SERIF, fontSize: 16, lineHeight: 1.6, margin: '8px 16px 4px', color: 'color-mix(in srgb, var(--foreground) 78%, transparent)' }}>
              A fight over a railroad junction — and, on the same ground, over what the railroad was for.
            </p>
            <CordTimeline>
              {SECTIONS.map(s => {
