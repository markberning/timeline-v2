...
// BATTLE dossier (Battle of Champion Hill). Same shape as Stones River / Shiloh: hero ·
...
const CRUMBS = civilWarCrumbs({ theatre: 'west', battleId: 'w-championhill' })
...
const TL_META: Record<string, { size: CardSize; date: string; palette: [string, string, string] }> = {
  'the-crossroads-hill': { size: 'l', date: 'Dawn', palette: ['#3a4a2a', '#283420', '#0a0e08'] },
  'the-jackson-road': { size: 'xl', date: '~1 p.m.', palette: ['#1d3a5a', '#16283a', '#080c12'] },
  'the-hill-changes-hands': { size: 'xl', date: '~2 p.m.', palette: ['#7a1422', '#3a1208', '#0a0606'] },
  'the-lost-division': { size: 'l', date: 'Dusk', palette: ['#3a2e21', '#2a221c', '#0a0806'] },
}
const SECTION_IMG: Record<string, string> = {
  'the-crossroads-hill': '/war-img/champion-hill-overview.png',
  'the-jackson-road': '/war-img/champion-hill.png',
  'the-hill-changes-hands': '/war-img/cmdr/bowen.jpg',
  'the-lost-division': '/war-img/champion-hill-retreat.png',
}
const sectionHref = (id: string) => `/war-civil-war/western/champion-hill/s/${id}`
  ...
  return (
    <div style={{ position: 'relative', height: 260, overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', left: 16, right: 16, bottom: 14 }}>
        ...
        <h1 style={{ margin: '6px 0 0', fontFamily: SERIF, fontWeight: 500, fontSize: 30, lineHeight: 1.05, letterSpacing: -0.5, color: '#fff' }}>Battle of Champion Hill</h1>
        <div style={{ fontFamily: SERIF, fontSize: 14, color: 'rgba(255,255,255,0.82)', marginTop: 4 }}>May 16, 1863 · Hinds County, Mississippi</div>
      </div>
    </div>
  )
}
      ...
      {open && (
        <div style={{ marginTop: 14 }}>
          <div style={{ display: 'flex', marginBottom: 18 }}>
            {([['Duration', '1 day'], ['Casualties', '~6,300'], ['Winner', 'Union']] as const).map(([k, v], i) => (
        ...
        {view === 'dossier' ? (
          ...
          <div style={{ padding: '8px 0 20px' }}>
            <p style={{ fontFamily: SERIF, fontSize: 16, lineHeight: 1.6, margin: '8px 16px 4px', color: 'color-mix(in srgb, var(--foreground) 78%, transparent)' }}>
              The decisive field battle of the Vicksburg campaign — a near-even fight Grant won by where he landed the blow.
            </p>
            <CordTimeline>
              {SECTIONS.map(s => {
