...
// BATTLE dossier (Battle of Franklin, 1864). Same shape as Shiloh: hero · collapsible
// At-a-glance (stat strip + armies face-off + casualties) · outcome card · commanders
// strip · numbered section list. Content via the war content pipeline
...
const CRUMBS = civilWarCrumbs({ theatre: 'west', battleId: 'w-franklin' })
...
const SECTION_IMG: Record<string, string> = {
  ...
  'the-cotton-gin': '/war-img/franklin-the-charge.png',
  'what-it-cost': '/war-img/cmdr/cleburne.jpg',
}
const sectionHref = (id: string) => `/war-civil-war/western/franklin/s/${id}`
  ...
  return (
    <div style={{ position: 'relative', height: 260, overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', left: 16, right: 16, bottom: 14 }}>
        ...
        <h1 style={{ margin: '6px 0 0', fontFamily: SERIF, fontWeight: 500, fontSize: 30, lineHeight: 1.05, letterSpacing: -0.5, color: '#fff' }}>Battle of Franklin</h1>
        <div style={{ fontFamily: SERIF, fontSize: 14, color: 'rgba(255,255,255,0.82)', marginTop: 4 }}>November 30, 1864 · Franklin, Tennessee</div>
      </div>
    </div>
  )
}
      ...
      {open && (
        <div style={{ marginTop: 14 }}>
          <div style={{ display: 'flex', marginBottom: 18 }}>
            {([['Duration', 'One day'], ['Casualties', '~8,600'], ['Winner', 'Union']] as const).map(([k, v], i) => (
