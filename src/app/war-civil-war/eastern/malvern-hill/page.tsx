...
// BATTLE dossier (Battle of Malvern Hill). Same shape as Antietam: hero · collapsible At-a-glance
...
const CRUMBS = civilWarCrumbs({ theatre: 'east', battleId: 'e-malvern' })

const HERO_IMG = '/war-img/malvern-hill-hero.jpg'
const ARMIES = [
  { side: 'Union', label: 'Army of the Potomac', size: '~54,000 troops', commander: 'Porter & Hunt', note: 'Massed scores of guns tier on tier on a commanding hill.', color: ACCENTS.blue },
  { side: 'Confederacy', label: 'Army of Northern Virginia', size: '~55,000 troops', commander: 'Gen. Robert E. Lee', note: 'Sent men uphill, piecemeal, into the teeth of the guns.', color: ACCENTS.rust },
]
...
const SECTION_IMG: Record<string, string> = {
  ...
  'richmond-saved': '/war-img/cmdr/dh-hill.jpg',
}
const sectionHref = (id: string) => `/war-civil-war/eastern/malvern-hill/s/${id}`
  ...
  return (
    <div style={{ position: 'relative', height: 260, overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', left: 16, right: 16, bottom: 14 }}>
        ...
        <h1 style={{ margin: '6px 0 0', fontFamily: SERIF, fontWeight: 500, fontSize: 30, lineHeight: 1.05, letterSpacing: -0.5, color: '#fff' }}>Battle of Malvern Hill</h1>
        <div style={{ fontFamily: SERIF, fontSize: 14, color: 'rgba(255,255,255,0.82)', marginTop: 4 }}>July 1, 1862 · Henrico County, Virginia</div>
      </div>
    </div>
  )
}
      ...
      {open && (
        <div style={{ marginTop: 14 }}>
          <div style={{ display: 'flex', marginBottom: 18 }}>
            {([['Duration', '1 day'], ['Casualties', '~8,500'], ['Winner', 'Union (tactical)']] as const).map(([k, v], i) => (
