...
// BATTLE dossier (Second Battle of Petersburg). Same shape as Antietam/Shiloh/Wilderness:
// hero · collapsible At-a-glance (stat strip + armies face-off + casualties) · outcome card ·
...
const CRUMBS = civilWarCrumbs({ theatre: 'east', battleId: 'e-petersburg2' })
...
const SECTION_IMG: Record<string, string> = {
  ...
  'the-hesitation': '/war-img/cmdr/wf-smith.jpg',
  'the-door-slams': '/war-img/cmdr/grant.jpg',
}
const sectionHref = (id: string) => `/war-civil-war/eastern/second-petersburg/s/${id}`
...
function HeroImg() {
  ...
  return (
    <div style={{ position: 'relative', height: 260, overflow: 'hidden' }}>
      {failed
        ? <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, #2a2640, #1c1a2e 55%, #080610)' }} />
      ...
      <div style={{ position: 'absolute', left: 16, right: 16, bottom: 14 }}>
        <Eyebrow color="#b9a8ee">Battle · Eastern Theatre</Eyebrow>
        <h1 style={{ margin: '6px 0 0', fontFamily: SERIF, fontWeight: 500, fontSize: 30, lineHeight: 1.05, letterSpacing: -0.5, color: '#fff' }}>The Second Battle of Petersburg</h1>
        <div style={{ fontFamily: SERIF, fontSize: 14, color: 'rgba(255,255,255,0.82)', marginTop: 4 }}>June 15–18, 1864 · Petersburg, Virginia</div>
      </div>
    </div>
  )
}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 16, fontFamily: SANS, fontSize: 11, color: 'color-mix(in srgb, var(--foreground) 65%, transparent)' }}>
        ...
        <span><span style={{ color: ACCENTS.rust }}>■</span> Confederacy ~3,200–4,000</span>
      </div>
    </div>
  )
}
      ...
      {open && (
        <div style={{ marginTop: 14 }}>
          <div style={{ display: 'flex', marginBottom: 18 }}>
            {([['Duration', '4 days'], ['Casualties', '~15,000'], ['Winner', 'Confederacy']] as const).map(([k, v], i) => (
