...
// American Revolution — an off-the-battlefield THEME chapter (client; renders the gated
// prose through the SAME shared <BattleSectionReader> the F&I/Civil War theme sections
// use, so cross-references become between-paragraph "Read the full story" pills). Prose
// comes from theme-narratives.ts (generated from the gated rev-<slug>-final.md by
// scripts/_build-rev-themes.mjs). An unbuilt theme shows a brief placeholder, never a 404.
...
const OTBF_HREF = '/war-revolution/off-the-battlefield'
...
export function ThemeLanding({ slug }: { slug: string }) {
  ...
  // Built + gated → the full chapter, rendered like an F&I theme section.
  if (narr) {
    return (
      <BattleSectionReader
        sections={{ main: narr }}
