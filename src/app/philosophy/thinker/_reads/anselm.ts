...
export const ANSELM: PhiNarr = {
  ...
  "brk": {
    ...
    "paragraphs": [
      "The inheritance Anselm worked from was Augustine's (era 2, chapter 1), and Augustine had set the terms five centuries earlier with a slogan of his own: *crede ut intelligas*, believe in order to understand. Faith comes first; understanding is its reward, granted to the believer who already holds the thing. Reason, in that picture, mostly defends and deepens a faith it does not establish. When earlier Christian thinkers did argue for God's existence, they argued the way most people still expect such arguments to run: from the world. The order is so vast, so beautiful, so evidently made, that it must have a maker. The argument starts outside, in the visible creation, and reasons back to its cause.",
