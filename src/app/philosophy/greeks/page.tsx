// "The Greeks" — the first Philosophy era, in a provisional self-contained reader modeled on
// the threads reader (src/app/threads/the-parties/page.tsx): its own light identity (a quiet
...
import { GREEKS, type PhiFig } from './narrative'
...
// Same parser as the threads reader; the Greeks prose uses only *italics*.
const INLINE_RE = /(\[([^\]]+)\]\((\/[^)]+)\))|(\*\*([^*]+)\*\*)|(\*([^*]+)\*)/
    ...
    <div className="phi-root">
      ...
      <main>
        ...
        <div className="phi-mast">
          <p className="phi-eyebrow">Philosophy &middot; The first era</p>
        ...
        <article id="phi-article" className="phi-body">
          ...
          <section className="phi-break" aria-label="Why this is a break">
            ...
            <div className="phi-break-row">
              ...
              <span className="phi-break-label">{n.brk.beforeLabel}</span>
            </div>
            <div className="phi-break-row">
              <span className="phi-break-tag phi-break-tag-after">After</span>
              <span className="phi-break-label">{n.brk.afterLabel}</span>
            </div>
