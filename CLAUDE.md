...
8b. **Write chapter intros (GATE — G13)** — `narratives/{tlId}.intros.json`, one entry per chapter: `bridge` ("where we are", ch≥2 only — ch1 has none), `setup` (situation + stakes, no outcome dump), `cast` (2–6 short `{name,note}` nameplates — a tag, NOT a definition; the prose still inline-defines on first use), `takeaway` (one sentence). It is the forward-looking framing card shown before the prose to cut reader overwhelm — so the **load-bearing rule is the inverse of a floor: keep it lighter than the chapter.** `tsx scripts/lint-intros.ts --tl={tlId} --strict` enforces presence + per-field word bounds + the **TOTAL ≤220-word cap** (an intro that becomes a second wall defeats its purpose). House voice, same as the narrative. Legacy 102 grandfathered in `audits/intro-baseline.json` (no intros yet — corpus retrofit is a separate scheduled program); **new civs are zero-tolerance**, same model as density/coverage. early-medieval-europe is the shipped pilot/reference.
...
scripts/
  ...
  lint-intros.ts                — G13 chapter-intro gate (presence + per-field bounds + TOTAL≤220 anti-density cap; baseline-grandfathered, --write-baseline)
public/
