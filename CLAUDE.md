...
10b. **Audit event popups (GATE — G10)** — `node --env-file=.env.local scripts/audit-events.mjs {tlId}`: Stage 1 text-coherence on every event (label/description vs wikiExtract/wikiSlug), Stage 2 vision on every event with a photo (image↔caption↔subject). **Calibration (locked 2026-05-17):** Stage 1 fails only a *genuinely wrong* page — disambiguation, same-name different-thing, redirect/off-subject; a **broad-but-on-subject parent article (same civ/era/thread) is a PASS** (thin-EN-coverage civs often have only a parent). Resolve a FAIL by, in order: (1) correct specific slug; (2) correct broad parent; (3) **drop the wikiSlug → blurb-only event** (the curated `description` is author-written + audited and renders standalone; *a missing link beats a wrong link*; G10 skips slug-less events); (4) waive in `content/.event-slug-waivers-{tlId}.json` (`{ "<eventId>": "reason" }`, mirrors G3 `.link-waivers`) for an on-thread parent that's a slight stretch. Image FAIL → fix the slug, `.image-overrides.json` / `.caption-overrides.json`, or drop the photo via `.image-rejections.json`. Fail-closed; writes `EVENT-FAILURES-{tlId}.txt`; re-run until clean.
10c. **Audit cross-links (GATE — G11)** — `node --env-file=.env.local scripts/audit-crosslinks.mjs {tlId}`: text-coherence on every cross-link — does the blurb describe the target chapter, is targetTl/targetChapter the era/topic-correct destination for the tapped subject? (`lint:links --strict` already hard-ERRORs an unresolvable targetTl/targetChapter.) Fail-closed; writes `CROSSLINK-FAILURES-{tlId}.txt`. Fix blurb / retarget / fix link, re-run until clean.
10d. **Audit glossary (GATE — G12)** — `node --env-file=.env.local scripts/audit-glossary.mjs {tlId}`: text-coherence on every glossary term. **Calibration (locked 2026-05-17, anti-dodge clause added 2026-05-17):** fails a *genuinely wrong* page (disambiguation / same-name different-thing / off-subject). A broad parent is a PASS **only if that page actually explains the tapped term**; a *specific named term* (a ceremony, rite, office, person, artifact, concept) pointed at a *whole-civilization / whole-empire / whole-dynasty* article that does **not** itself explain that term is a **FAIL — it is a dodge**, not an acceptable parent, even though the empire is "same civ/era/thread." The reader who taps "ʻinasi" and lands on "Tuʻi Tonga Empire" learns nothing about ʻinasi; that is the failure. "On-thread" is necessary but not sufficient — the page must *cover the term*. **"No good EN article" resolution order (canonical, locked 2026-05-17):** (1) **authored `definition` blurb with no `wikiSlug`** — parse normalizes it to a `def:` token, the GlossarySheet shows house-voice prose and NO Wikipedia link, and G12 SKIPS it (auto-PASS, like G10 skips slug-less events). **This is the preferred answer** — it scales to thin-EN-coverage civs without degrading the tap; prefer it over pointing a niche term at the whole-civilization article. (2) correct broad on-thread parent slug. (3) residual waived per-term in `content/.glossary-slug-waivers-{tlId}.json` (`{ "<term>": "reason" }`, case-insensitive) for a broad-parent stretch. Glossary terms hit "no specific EN article" far more than events ("merit land", "Daesik"…) — an authored `definition` is the normal answer; parent/waiver only when a real parent genuinely exists. (`lint:links` already ERRORs empty/dead slugs; `def:` tokens are exempt.) Fail-closed; `GLOSSARY-FAILURES-{tlId}.txt`.
...
14. **Ship toggle (GATE)** — `node scripts/ship-check.mjs {tlId}` must pass, THEN flip `hasContent: true` in `src/lib/navigator-tls.ts`. ship-check verifies: maps exist 1:1 with chapters + QA-passed, event popups QA-passed (G10), cross-links QA-passed (G11), glossary QA-passed (G12), links 0-ERROR + targets resolve, coverage triaged, density in band, no Persona-D WEAK/REWRITE, backward pass done.
...
content/                        — curated link files + generated JSON
  ...
  .link-waivers-{tlId}.json    — G3 bolded-but-unlinked waivers ({chapter:[term]})
