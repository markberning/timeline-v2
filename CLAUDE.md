...
src/
  ...
  components/
    ...
    narrative-reader.tsx        — client wrapper for event + glossary + cross-link click delegation; reads ?chapter=N and ?highlight=term&snippet=text on mount for search-to-highlight
    ...
    chronology/
      ...
      chronology-header.tsx     — "Stuff Happened" eyebrow + "The Civ Lib" title + search icon + dark mode toggle
      search-overlay.tsx        — full-screen search: loads search-index.json, groups results by civ → chapter → sentence with highlighted query term, navigates to exact match
  ...
  lib/
    ...
    accent-colors.ts            — region-driven accent colors (one color per region) with WCAG-safe text/badge variants
    categories.ts               — event category metadata (colors for 8 categories)
    navigator-tls.ts            — 91 navigator TLs with region, startYear, endYear, subtitle (descriptive tagline), hasContent flag (true for 39 shipped TLs)
...
scripts/
  parse-narratives.ts           — markdown → JSON build pipeline (two-pass: collects TL metadata first, then injects cross-links before event/glossary; ALSO emits per-TL offline manifests to public/offline/{tlId}.manifest.json AND generates public/search-index.json for full-text search)
...
public/
  ...
  search-index.json             — generated by parse; plain-text sentences for all shipped TLs, ~4.1 MB, lazy-loaded by search overlay
...
- **hasContent dimming** — rows with `hasContent: true` (39 shipped TLs) render at full opacity; others at 0.35.
...
- **Globe (`/globe`)** — D3 orthographic SVG globe with 86 civilizations, consolidated from the former `/globe2` route. Old WebGL globe.gl (`globe-view.tsx`) is dead code. Key features:
  ...
  - **Globe ↔ Navigator sync** — 86 globe civs mapped to 91 navigator TLs via `GLOBE_TO_READER` in `globe2-data.ts`. Sub-civs show parent name in parens (e.g. "Silla / Unified Silla (Ancient Korea)"). All 39 shipped TLs reachable from globe.
...
- **Full-text search (home page)** — magnifying glass icon in header opens a full-screen overlay. Searches all 39 shipped TLs (~650k words). Results grouped by civ (region-colored header) → chapter (number + title) → matching sentences with query term highlighted in bold. Tap a result → hard-navigates to `/{tlId}/?chapter=N&highlight=term&snippet=context`, which auto-expands the chapter, scrolls to the exact paragraph, and highlights the line containing the match with an amber overlay that persists until the user scrolls or touches. Search index (`public/search-index.json`, ~4.1 MB) generated at build time by `parse-narratives.ts`, lazy-loaded on first search activation.
- **Globe era labels** — info card era strip has tiny labels under each segment (Bronze Age, Iron Age, Classical, Medieval, Early Modern) so it reads as historical eras, not chain position.
...
1. ✅ mesopotamia — 13 chapters, fully audited (2026-04-24 re-audit: 10 STRONG + 3 GOOD→STRONG after structural fixes to Ch 8/9/13; 12 factual precision fixes from Skeptic pass including Eridu temple count, Uruk population range, Sennacherib plague pathogen, NYC-Detroit distance, Etemenanki height range), 82 curated event links, **336 glossary links**, **121 summary bullets across 13 chapters**, **34 cross-links**, 84/89 images (95%), 13 WebP chapter maps, label "Mesopotamia"
...
4. ✅ post-maurya-kingdoms — 8 chapters (~15k words), label "Post-Maurya Kingdoms", 69 reference events, 60 event links, 195 glossary links, 17 cross-links, 8 chapter maps pending, full audit + fixes. Covers Shunga coup through Kushan Empire to Gupta founding (-185 to 320 CE). Central thesis: absorption was the pattern — every conqueror got to rule, India defined the rules.
5. gupta-empire
6. medieval-india
7. delhi-sultanate
8. mughal-empire
...
5. ✅ han-dynasty — 8 chapters (~15k words), label "Han Dynasty", 70 reference events, 76 event links, 199 glossary links, 16 cross-links, 66 summary bullets, 8 chapter maps pending, full audit + fixes. Covers Liu Bang through Yellow Turbans to Three Kingdoms split (-206 to 220 CE). Central thesis: China's silk-road golden age — the dynasty that made Confucianism the operating system and the Silk Road the supply chain.
6. six-dynasties
7. tang-song-china
8. yuan-dynasty
9. ming-dynasty
...
2. ✅ andean-kingdoms — 8 chapters (~15k words), label "Andean Kingdoms", 65 reference events, 71 event links, 194 glossary links, 14 cross-links, 8 chapter maps pending, full audit + fixes. Covers Paracas weavers through Moche warrior-priests to Tiwanaku (-200 to 500 CE). Central thesis: the age of warrior-priests — where Chavín had religion, the regional cultures invented politics backed by blood.
...
3. ✅ zapotec-civilization — 8 chapters (~14.7k words), label "Zapotec", 68 reference events, 63 event links, 180 glossary links, 8 WebP chapter maps, full audit + fixes. Covers Oaxaca's mountaintop scribes from Monte Albán's founding through Spanish contact (-1500 to 1521 CE). Central thesis: Oaxaca's scribes invented writing in the Americas and ruled from a mountaintop for 1,300 years.
4. ✅ teotihuacan — 8 chapters (~15k words), label "Teotihuacan", 65 reference events, 64 event links, 189 glossary links, 14 cross-links, 66 summary bullets, 8 chapter maps pending, full audit + fixes. Covers the anonymous metropolis from volcanic refugees through Aztec mythology (-100 to 550 CE). Central thesis: the greatest anonymous civilization in human history — 200,000 people governed by people whose names we will never know.
5. aztec-empire
...
**Central Asian Steppe chain** (in progress):
1. ✅ scythians — (see standalone entry below, also chain member)
2. ✅ xiongnu-huns — 8 chapters (~13.5k words), label "Xiongnu", 65 reference events, 65 event links, 200 glossary links, 8 WebP chapter maps, full audit + fixes. Covers Modu Chanyu's steppe empire through Southern Xiongnu dissolution (-209 to 216 CE). Central thesis: the nomad empire that forced China to build the Wall and invent the Silk Road.
3. gokturk-khaganate
4. mongol-empire
5. timurid-empire

**Northern European chain** (in progress):
1. ✅ celtic-cultures — 8 chapters (~15k words), label "Celtic Cultures", 65 reference events, 65 event links, 200 glossary links, 8 WebP chapter maps, full audit + fixes. Covers Hallstatt through Gallic Wars to insular Celtic survival (-800 to 400 CE). Central thesis: a civilization defined by what it wasn't — no empire, no writing, no capital, but a cultural zone from Ireland to Turkey.
2. viking-age

**Japanese Civilization chain** (in progress):
1. ✅ ancient-japan — 8 chapters (~13.8k words), label "Ancient Japan", 65 reference events, 65 event links, 200 glossary links, 8 WebP chapter maps, full audit + fixes. Covers Jōmon pottery through Genpei War (-14000 to 1185 CE). Central thesis: the world's oldest pottery makers became the world's most refined courtiers, then the courtiers lost everything to the warriors.
2. shogunate-japan
3. meiji-japan

**Standalone TLs (no chain):**
- ✅ phoenicia — 8 chapters (~16k words), label "Phoenicia", 71 reference events, 69 event links, 192 glossary links, 18 cross-links, 63 summary bullets, 8 WebP chapter maps, full audit + fixes. Covers Levantine coast from Bronze Age through destruction of Carthage (-1500 to -146 BCE). Central thesis: never built an empire — built something more durable: the alphabet, the trade routes, the colonies.
- ✅ polynesian-voyagers — 8 chapters (~16k words), label "Polynesian Voyagers", 70 reference events, 70 event links, 214 glossary links, 17 cross-links, 67 summary bullets, 8 WebP chapter maps, full audit + fixes. Covers Lapita expansion through Polynesian Triangle settlement (-1500 to 1300 CE). Central thesis: the greatest exploration in human history, without metal, writing, compasses, or maps.
- ✅ ancient-israel — 8 chapters (~16k words), label "Ancient Israel", 71 reference events, 71 event links, 199 glossary links, 15 cross-links, 67 summary bullets, 8 WebP chapter maps, full audit + fixes. Covers Hebrew kingdoms from Saul through Babylonian exile (-1020 to -586 BCE). Central thesis: a tiny kingdom in a terrible location that produced the most influential collection of texts in human history.
