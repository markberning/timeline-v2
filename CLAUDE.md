...
14. **Ship toggle (GATE)** — `node scripts/ship-check.mjs {tlId}` must pass, THEN flip `hasContent: true` in `src/lib/navigator-tls.ts`. ship-check verifies: maps exist 1:1 with chapters + QA-passed, event popups QA-passed (G10), cross-links QA-passed (G11), links 0-ERROR + targets resolve, coverage triaged, density in band, no Persona-D WEAK/REWRITE, backward pass done.
