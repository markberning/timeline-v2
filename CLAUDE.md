...
10d. **Audit glossary (GATE — G12)** — `node --env-file=.env.local scripts/audit-glossary.mjs {tlId}`: text-coherence on every glossary term. **Same calibration as G10 (locked 2026-05-17):** fails only a *genuinely wrong* page (disambiguation / same-name different-thing / off-subject); a broad-but-on-subject parent is a PASS. Residual waived per-term in `content/.glossary-slug-waivers-{tlId}.json` (`{ "<term>": "reason" }`, case-insensitive). Glossary terms hit "no specific EN article" far more than events ("merit land", "Daesik"…) — the parent or a waiver is the normal answer. (`lint:links` already ERRORs empty/dead slugs.) Fail-closed; `GLOSSARY-FAILURES-{tlId}.txt`.
