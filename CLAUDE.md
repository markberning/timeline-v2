...
10d. **Audit glossary (GATE — G12)** — `node --env-file=.env.local scripts/audit-glossary.mjs {tlId}`: text-coherence on every glossary term — is the (alive) wikiSlug/extract actually the right subject, not a disambiguation / same-name different-thing / over-broad redirect? (`lint:links` already ERRORs empty/dead slugs.) Fail-closed; `GLOSSARY-FAILURES-{tlId}.txt`.
