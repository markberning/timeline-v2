...
| 18 | **Corpus FULL subject-confirm under the new link process** | 2026-05-17 link-verification redesign (`memory/project_link_verification_redesign`) | every existing civ's event+glossary link **(a)** mechanically de-broken (dead/disambig/redirected fixed) **AND (b)** its page subject-confirmed by a human/agent eyeballing title+lead vs the term/event — plausible-but-wrong pages fixed — THEN snapshotted. The strong guarantee, NOT the bootstrap. No model spend; substantial attention. | ☑ **DONE 2026-05-17** — executed via the no-AI `fix-links`/`retarget-links`/`audit-retargets` tools + 8 parallel resolver agents + coordinator safety sweep. 801 flagged links across 99 civs: 481 retargeted to the correct page, 208 → authored house-voice definitions, 111 confirmed false-alarm synonyms left alone. 2,340+ recycled/off-topic pictures tidied. lint:links --strict 0 ERROR; corpus broken-link 801→111 (residual = correct pages under a synonym). Shipped + deployed (`8334ae9`). Snapshot-write step still owed if the deterministic ship gate is to enforce it. |
