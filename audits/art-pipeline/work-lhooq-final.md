# WORK FINAL — Marcel Duchamp, *L.H.O.O.Q.* (1919)

Reconciled draft. Every [BLOCKER]/[FIX] from the fact, read, and frame gates folded:
- **FACT B1** — the verbatim "becomes a man" quote re-attributed to Duchamp's 1961 radio interview with Herbert Crehan (WBAI, New York; published in *Evidence*), NOT the Sweeney 1955/56 interview / *The Writings of Marcel Duchamp* p.133. Quote body unchanged (keeps the load-bearing "not").
- **READ** — reader-commands, meta-narration, and honesty-labels stripped throughout; flattened to declaratives. Facts unchanged.
- **FRAME C-1 / F-1** — Sapeck / the Incoherents (1883 pipe-smoking *Mona Lisa*) precedent added; lone-inventor frame punctured while keeping the real rupture (the readymade *claim*).
- **FRAME C-3** — Rrose dating tightened ("the very next year").
- Appropriation claims left correctly hedged ("early seed," "one of the first"); pun-vulgarity and gender-as-documented-intent handling unchanged.

Two fenced blocks: PART A (the `ArtWorkContent` const) and PART B (the five `Lho…` section components + the `NARRATIVES` registry line). Identifiers identical to the draft.
