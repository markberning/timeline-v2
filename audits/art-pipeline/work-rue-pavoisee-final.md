# FINAL — Raoul Dufy, *La Rue pavoisée* (Street Decked with Flags), 1906 · WORK read

Resolved from `work-rue-pavoisee-draft.md` by folding the gated fixes in. FACT and FRAME
gates were CLEAN; this pass applies the READ gate's FIXes + selected NICEs:
- BREAK Monet contrast compressed so it no longer re-runs LOOKING's scissors beat (now lands only the unique fleeting-moment vs. build-the-space point).
- LOOKING "Up the street": "tumbling down toward you" replaced so the motion runs up-and-away with the established recession.
- LOOKING crowd: concrete what-to-look-for cue added (small dark/light dabs massed at the base of the street).
- "high key" introduced and glossed once in LOOKING's palette paragraph; the bare "high-keyed" in MAKING swapped to plain words so nothing lands cold.
- "aplats" gloss confirmed inline wherever it appears (PART A annotation).
- NICEs: the later textile work / Bianchini-Férier named in AFTERLIFE prose; the lone "most important colorist in France" superlative softened; one "festival doing the Fauve's work" hit reworded.

Imperial dimensions only; new prose uses parentheses/commas, never the em-dash character;
verbatim-quote punctuation preserved; no new unverified facts.

**LEGEND / handling notes (unchanged from draft):**
- Matisse's *Luxe, calme et volupté* (1905 Indépendants) is stated as **effect**, NOT a flat quote.
- The street is **"a Le Havre street decked for the 14th of July,"** never named "Rue des Drapiers."
- This is the **1906 Pompidou (MNAM) AM 4113 P** canvas only — NOT the 1907 MoMA, NOT the NGA Washington July-14 canvases.
- **No lettering/signs annotation** (unconfirmed); the 6th pointer is the high-key palette.
- Monet's 1878 flag-street fête painting named **generically**.
...
function RueLooking({ accent }: { accent: string; onZoom: (src: string, cap: string) => void }) {
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseStyle}>
        And the key the color is pitched in is the last thing to take in: a <strong>high, festive register</strong>, vivid reds, blues, greens and pinks at close to full strength, the opposite of muted Impressionist tone. Painters call this <strong>high key</strong>, meaning the whole palette pitched bright and light-filled, near full strength, with the darks kept out, and high key is exactly what carries the celebration. You can feel the holiday in the painting before you have named a single object in it, because the brightness is doing the talking. That is a Fauve picture: a street remade as a chord of pure, loud, flat color, struck once, on a summer holiday, in a port town that happened to have raised the painter who could hear it.
      </p>
    </article>
  )
}
...
function RueBreak({ accent }: { accent: string; onZoom: (src: string, cap: string) => void }) {
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseStyle}>
        Set it against the older way to see the size of the break. <strong>Monet&rsquo;s</strong> flag-decked street of 1878, the obvious ancestor of this picture, used the same motif and the same holiday to chase one thing above all: a fleeting <em>moment</em>, the tricolors caught mid-flutter, color recording a passing instant of breeze and light. Dufy turns that purpose inside out. He uses the identical subject not to capture a moment but to build the space of the picture outright, the color, as the French study of the canvas puts it, &ldquo;independent of the subject&rdquo; and generating the space on its own. Same flags, opposite job. That is the move that points past Impressionism altogether, toward a painting that is frankly about its own flat colored surface, and a small flagged street in Le Havre is where Dufy made it, plainly, on a holiday afternoon.
      </p>
    </article>
  )
}
