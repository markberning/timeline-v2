# Work draft — Clyfford Still, *1957-D No. 1* (1957) — id `still-cliff`

Authored through the art content pipeline from `audits/art-pipeline/work-still-cliff-factpack.md` (source of truth). Abstract Expressionism work-read, chain index 6 of 9. Rights: `in-copyright` (Still d. 1980), shown small under fair use. Wired image confirmed against the file: a black-dominant field torn by jagged yellow/beige verticals, white accents, bare-canvas patches lower-left, landscape (≈1.40). Date-code title (NOT a "PH" number); the color quote ends "…fuse into a living spirit" and is kept separate from the "life and death merging in fearful union" statement; "life-lines" avoided as a Still term.
...
export const STILL_CLIFF: ArtWorkContent = {
  ...
  hook: 'A wall of black, nine by thirteen feet, troweled on with a palette knife and torn through by jagged shards of yellow, painted by the most combative man in American art, who refused titles, quit the galleries, and sealed away most of what he made.',
  ...
  sections: [
    { id: 'independent', eyebrow: 'New York · 1950s', dateLabel: '~1952–1959', title: 'The painter who quit the room', blurb: 'Still was the most combative of the New York abstractionists. He cut ties with commercial galleries, stopped showing in public for about seven years, refused descriptive titles, and would later seal most of his life’s work away from view entirely.', progress: 0.08 },
...
function StlMaking({ accent }: { accent: string; onZoom: (src: string, cap: string) => void }) {
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseStyle}>
        So the making and the meaning are the same fact. The troweled crust, the torn edges, the refusal of a composition: each is a way of insisting that the painting is not a designed picture of anything but a raw, continuous thing in its own right. With that in mind, stand in front of it and look at what is actually there.
      </p>
    </article>
  )
}
...
function StlBreak({ accent }: { accent: string; onZoom: (src: string, cap: string) => void }) {
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseStyle}>
        Read it against the canvas and it lines up exactly. Not color as decoration, not texture as effect, not shapes to be arranged, but black and yellow and crusted paint fused into one charged, continuous thing. (His other famous line, that his pictures were &ldquo;life and death merging in fearful union,&rdquo; is a separate statement, made about other work, and it is not the same sentence as this one, so we keep them apart.) What follows from the break is the movement it helped open: the torn, full-bleed color field, alongside Rothko and Newman, pointed the way to <strong>Color Field painting</strong> and to abstraction as a total environment you stand inside, a moral and existential claim rather than a designed picture.
      </p>
    </article>
  )
}
