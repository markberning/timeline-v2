# FINAL — Richard Hamilton, *Just what is it that makes today's homes so different, so appealing?* (1956)

Work-level read for the Pop Art chain. Reconciled from the draft + 4 gates (fact / read / frame / factpack). All [BLOCKER] and [FIX] findings folded.
...
- FRAME [BLOCKER-1]: the Alloway "in print around 1958" claim was wrong — his Feb 1958 *Architectural Design* essay used "mass popular art," not "Pop art." Rewritten: term grew up inside the IG; Alloway carried it into wider use; the earliest print appearance of "pop art" is the Smithsons' 1956 *Ark* piece; first print use of "Pop" by Alloway pinned to *Architectural Design*, Feb 1958, but scoped to NOT being where "Pop art" the term first appeared. POP-named-the-movement claim stays scoped to Alloway/IG and debunked (Paolozzi 1947 first carried "POP").
- FRAME [FIX-1] + FACT [FIX-1] + confirmed directive: McHale's contribution hedged as DISPUTED across every surface (header, JwiCutting, JwiTomorrow team add, provenance, figures card) — his family claimed a larger hand in the design; Hamilton played it down / called it absurd. Never assert a clean split.
- FRAME [FIX-2]: provenance note brought into line with the hedge.
- READ [FIX]: Ford emblem folded into the brand-marks clause (no false-precision location); reader-command "just register" cut; honesty-label "the honest answer is" cut; meta "A note on the text" de-metaed; "three years before 'Pop art' became common usage" dropped (unsupported dating); 4th letter-paper repeat trimmed; School of Paris glossed; "first surprising fact" meta hook softened; "Look at it as what it pretends to be" softened.
- NICE adds taken: *This Is Tomorrow* collaborating-teams clause (FRAME NICE-1, doubles as McHale-fairness setup); title-is-a-found-ad-headline point (FRAME NICE-2).
- Em-dash contract: Part A string fields carry NO literal "—" (commas/parens/colons; en-dash only in heroAspect comment range). Part B JSX uses &mdash; / &ndash; entities. Hamilton 1957 list verbatim.
