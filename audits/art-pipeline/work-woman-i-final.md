# FINAL — *Woman I*, Willem de Kooning (1950–52) — work-read

Reconciled from `work-woman-i-draft.md` against the three gates (fact PASS; read 4 FIX; frame 2 FIX + named NICEs folded). All [BLOCKER]/[FIX] items folded; FACT gate carried only trivially-correct NICE (none required a change). Voice/format contract enforced: no meta-narration / reader-commands / honesty-labels / condescending glosses; no literal "—" in rendered strings; plain TS fields use commas/parens/colons; JSX may use `&mdash;`; the verbatim "Mesopotamian idols … with this smile" quote is preserved exactly. medium = oil and metallic paint on canvas; rights `in-copyright`; dimensions ft/in; heroAspect 0.76, heroFit 'contain'. Identifiers, section ids, and component names are identical to the draft.

Changes folded:
- **READ.** Cut the tour-guide imperatives in `WmnLooking` ("Stand in front of it" / "Go to the face" / "Now the body" / "Then look at how it is painted") → declarative pointers. Removed the four "honest" uses and the sourcing throat-clearing in `WmnMaking` + `WmnAfterlife` (the account-not-gospel caution stays, folded into the telling, never announced). Trimmed the overlong "flesh"-line parenthetical.
- **FRAME.** The "influence" facet now NAMES successors (Grace Hartigan, Larry Rivers, Robert Rauschenberg + the *Erased de Kooning Drawing*, the Pop line) in `WmnBreak`. "the most prominent of the Abstract Expressionists" → "one of the most prominent" in `WmnAfterlife`. Named Carol Duncan ("The MoMA's Hot Mamas") for the feminist critique. Trimmed the triple "betrayal" drumbeat (kept one attributed instance in `WmnFigure`, one "apostasy" in `WmnAfterlife` paired with the contemporary pro-view; cut the "betrayal" repeat from `WmnFigure` ¶2 to a single, clearly-attributed beat). Misogyny stays a balanced, unadjudicated debate.
- Tightened `WmnFigure` ¶3 structural runway ("The rest of these chapters are about…") to drop the mild self-framing.
