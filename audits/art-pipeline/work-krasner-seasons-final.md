# FINAL — Lee Krasner, *The Seasons* (1957) · work-read `krasner-seasons`

Reconciled from `work-krasner-seasons-draft.md` against the three gates
(`gate-fact`, `gate-read`, `gate-frame`) and `factpack`. Last work in the
Abstract Expressionism chain (9 / 9). Rights: **in-copyright**.

Folded this pass:
- **FACT FIX-1** — "a damn good painter, thank you, and a little too independent" is genuine
  but was misattributed to "Pollock-Krasner House materials." Corrected to biographer **Gail
  Levin**, recounting Krasner from their first meeting in 1971, in BOTH the const header note
  and the `KraAfterlife` prose. ("This was my answer" stays framed as reported, AAA/Seckler.)
- **FACT FIX-2** — `figures` Barbara Rose: was "Critic; her 1983 retrospective." She was the
  **curator**, and the retrospective opened at MFA Houston then ran at MoMA Dec 1984–Feb 1985
  (after Krasner's death). Loosened the date and fixed the role.
- **READ** — LOOKING reader-command pile-up reduced to a single deliberate scene-set; the
  KraMaking date parenthetical rewritten declarative (dropped "Hold the date" / "Do not let").
- **FRAME** — Krasner kept the agent of her own work throughout; "largest … to that point"
  made uniform (NICE-3) so no instance reads as a flat "largest of her life" overclaim.
