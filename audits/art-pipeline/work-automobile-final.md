# FINAL — Luigi Russolo, *Dynamism of an Automobile*, 1912–13 (work read)

Reconciled from `work-automobile-draft.md` against all three gates (fact / read / frame),
folding every [BLOCKER] and [FIX]. Source of truth: `work-automobile-factpack.md`.
Imperial dims only; no literal `—` in new prose (`&mdash;`/`&ndash;` in JSX; verbatim-quote
punctuation preserved); born-verified figures only. Date = **1912–13** (canvas signed "1911" [sic]).
NOT committed, NOT built. Identifiers/section-ids/component names UNCHANGED from the draft.
...
**Coordinator gate-time dependency (fact FIX-1, unresolved until the image lands):** `ART_IMG.russoloAutomobile` is not yet in the repo. Before lock, open the born-verified hero image and confirm every directional/shape `where` in the six annotations and the looking chapter: (a) the car/compression sits on the LEFT and motion runs rightward; (b) the chevron force-lines read red; (c) the "denser knot from which the wedges spring" placement; (d) the "flat stepping bands" of color. If the image confirms left-to-right thrust, the looking chapter MAY sharpen "one way" to "left to right" in one place (see the in-chapter marker). If the image contradicts any `where`, fix the annotation. Until then these stay image-agnostic by design.
...
| Manifesto glorified war as “world’s only hygiene” + “scorn for woman” (Point 9); Marinetti later co-founded Fascism | brief + FUTURISM hookLong; frame FIX-2 | DOCUMENTED (one proportionate line; verbatim Point-9 phrase) |
...
| Grapeshot = clustered iron shot from a cannon (gloss outside the quote) | read gate clarity FIX | DOCUMENTED gloss |
...
| Wedges/arrowheads driving one way; landscape format | annotations 1/3/5 | descriptive — VERIFY ON IMAGE at gate-1 (left/right agnostic until then) |
| Chronophotograph / Muybridge galloping horse (strip-of-film gloss) | read gate clarity FIX; lineage parent | DOCUMENTED (Muybridge standard attribution) |
| Color in flat stepping planes, stack-of-stills | annotation 4 | VERIFY ON IMAGE |
| Balla painting the same speeding car into near-abstraction, 1913 | Beat 4; frame NICE-1 | DOCUMENTED parallel (not influence-claim) |
...
| Influence on Varèse, Cage, Schaeffer; founding ancestor of noise/musique concrète/electronic | Beat 5 ⟦S3⟧⟦S4⟧ | DOCUMENTED (“a founding ancestor,” not “the father of”) |
| Shown Florence 1913–14 (Lacerba / Galleria Gonnelli) + Futurist group London (Doré Gallery) 1914; variant titles | §5, §1; fact FIX-2 | DOCUMENTED (canvas’s individual London presence NOT over-asserted) |
| Provenance gap Russolo→Sonia Delaunay; Robert-Delaunay acquisition noted but uncorroborated | §5 GAP flag; frame FIX-1 | GAP — hedged, NOT asserted as fact |
...
**Outstanding for coordinator (gate-time, fact FIX-1):** `ART_IMG.russoloAutomobile` does not yet exist in the repo. Before lock, open the born-verified hero image and re-check every left/right/which-shape `where` in the six annotations AND the looking chapter (wedge direction, dense-core/spring location, landscape thrust, red force-lines, flat stepping color) against the pixels. The standard reading is car/compression on the LEFT, motion rightward, red arrows; if the image confirms it, the marked spot in the looking chapter MAY sharpen "one way" to "left to right". If it contradicts any `where`, fix that annotation. Confirm the Marinetti translation matches the single cited published source (Flint/Britannica, ⟦S6⟧/⟦S7⟧) letter-for-letter, or convert to paraphrase per born-verified doctrine.
