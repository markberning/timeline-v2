# DRAFT — André Derain, *Charing Cross Bridge, London* (1906) — WORK READ

**Author (Opus) draft, gated art pipeline step 3.** Written ONLY from
`work-charing-cross-factpack.md`. Five sections, six annotations, ~5 figures,
provenance with no invented prices, lineage. PART A = paste-ready const for
`src/lib/art-content.ts`. PART B = five `Crx`-prefixed reader components
(absinthe voice) for `art-section-reader.tsx`.

**Key handling honored:** NGA Washington / John Hay Whitney Collection / acc.
1982.76.3 (NOT MoMA's *Charing Cross Bridge* or *London Bridge*). Dimensions
imperial only (2 ft 7 5/8 in × 3 ft 3 1/2 in). Series count hedged ("dozens
commissioned, around thirty delivered"); trip count hedged. The spanned bridge
is the Charing Cross / Hungerford railway bridge; Westminster and Big Ben are on
the far bank, never called the spanned bridge. The "translate, not reproduce"
idea is PARAPHRASED, never quoted. New prose uses parentheses/commas, never the
em-dash.
...
export const CHARING_CROSS: ArtWorkContent = {
  ...
  sections: [
    { id: 'assignment', eyebrow: 'Paris · 1906', dateLabel: '1906', title: 'A dealer sends him to beat Monet', blurb: 'The Paris dealer Ambroise Vollard ships a 26-year-old Fauve to London on purpose, to paint a Thames series that can rival the Monet London paintings everyone in Paris had just gone mad for.', progress: 0.08 },
...
function CrxSubject({ accent }: { accent: string; onZoom: (src: string, cap: string) => void }) {
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseStyle}>
        He did not, however, paint most of these on a London riverbank. Like a lot of the series, this kind of canvas was worked up <strong>back in France</strong> from sketches and drawings made in London, with only some of the group painted in front of the motif. That matters, because it tells you the color was never a transcription of what stood in front of him. A painter copying the real Thames on the spot, in changing light, makes one kind of decision. A painter back in his French studio, building a London out of sketchbooks and memory, is free to make the river any color he likes. Derain took that freedom all the way, which is the subject of the next section.
      </p>
    </article>
  )
}
  ...
  return (
    <>
      <article style={{ padding: '18px 18px 40px' }}>
        ...
        <p style={proseStyle}>
          This particular canvas had a long and well-traveled life as an object. It went from Derain to Vollard in 1906, and then began a journey through some of the biggest names in the 20th-century art market. The American collector <strong>John Quinn</strong> bought it in 1918. After his estate it probably passed to the Paris dealer Paul Guillaume, then through the London and Paris trade, then to the New York firm <strong>Knoedler</strong>, and possibly into the collection of <strong>Walter P. Chrysler, Jr.</strong> (the &ldquo;probably&rdquo; and &ldquo;possibly&rdquo; are the museum&rsquo;s own honest hedges, kept here on purpose). At the Chrysler sale in New York in February 1950 it changed hands again.
        </p>
        ...
        <p style={proseStyle}>
          That is the whole arc, worth stating flat. A dealer&rsquo;s commercial bet sent a 26-year-old to London to out-paint Monet; the painter came back with a river the color of a sunrise that never happened; the canvas wandered for forty years through the great collections of two continents; and it ended up on a wall in Washington, where people who have never heard of Ambroise Vollard stand in front of a pink Thames and feel, exactly as Derain wanted, something the real river could never have made them feel.
        </p>
      </article>
      ...
      <MeanwhileSheet
        ...
        body="While Derain is painting his pink Thames, a 25-year-old Pablo Picasso is at work in a Montmartre studio on a large, ugly, deliberately shocking canvas of five women, the picture that will become Les Demoiselles d’Avignon. Fauvism’s liberation of color is barely three years old and already the next revolution is starting a few streets away, this one aimed not at color but at form. The wild beasts had freed the palette; Picasso and Braque were about to dismantle the whole idea of a single, fixed view, and Cubism would soon make Fauvism look almost gentle."
...
| Vollard early shows Cézanne/Picasso | general dealer fact (Vollard standard); SAFE characterization | DOCUMENTED-general |
...
| "dozens commissioned, around thirty delivered" | §7 [DISPUTED] | HEDGED per pack — no hard number |
...
| Divisionism (Seurat/Signac) loosened spots + flat Fauve patches | Annotation 6 / Beat 3 | DOCUMENTED |
| "translate not reproduce" PARAPHRASED, no quote marks | Beat 3 [DISPUTED-as-verbatim] | PARAPHRASE only — per pack |
...
| Meanwhile: Picasso Les Demoiselles 1906–07, Montmartre, next revolution = form | cross-vertical, matches existing DEMOISELLES const | DOCUMENTED (app-internal) |

**Gate-6 confirm-by-eye list (coordinator):** bridge placement; river hues (no blue/gray, over bare canvas); sky hues; barge count/placement (prose hedges to "small dark barges," safe); Big Ben identifiable in the chosen scan; both touches (dots + flat patches) visibly coexist. Use the higher-resolution correct-color scan; fact pack flags the `.png` resolution as unconfirmed and offers the NGA open-access JPG as an alternate.
