# DRAFT — Giorgio de Chirico, *The Song of Love* (1914)

Work-read for the Surrealism chain. PART A = the `SONG_OF_LOVE` const for
`src/lib/art-content.ts`. PART B = the five `Sng*` section components for
`art-section-reader.tsx` + the trailing NARRATIVES registry comment.

Authored from `audits/art-pipeline/work-song-of-love-factpack.md` (source of truth).
Factpack corrections honored: de Chirico is a FORERUNNER, not a Surrealist (this is
1914 Metaphysical painting, a decade before the 1924 movement); the glove reads
pink/red (Apollinaire called it pink); Magritte saw a REPRODUCTION in 1922 (shown by
Marcel Lecomte), not the canvas; the "Et quid amabo nisi quod aenigma est" motto is
on a DIFFERENT 1911 self-portrait and is NOT attached to this painting; the marble
head as the Apollo Belvedere is a hedged secondary ID (MoMA says generic "Greek head").
...
export const SONG_OF_LOVE: ArtWorkContent = {
  ...
  sections: [
    ...
    { id: 'afterlife', eyebrow: 'After', dateLabel: '1922–today', title: 'The reproduction that turned Magritte', blurb: 'In 1922 the poet Marcel Lecomte showed René Magritte a reproduction of this picture, and Magritte said his eyes saw thought for the first time. The canvas passed to Nelson Rockefeller, who bought it in 1950 and left it to MoMA in 1979, where it hangs now.', progress: 0.96 },
  ],
  provenance: [
    ...
    { year: 'c. 1950', who: 'Nelson A. Rockefeller', place: 'New York', note: 'Bought by the American collector and politician Nelson A. Rockefeller (from the dealer Marcel Raval, reportedly in October 1950). No purchase figure is documented in the sources, so it is left blank rather than invented.', price: null },
  ...
  annotations: [
    ...
    { label: 'The little train on the horizon', where: 'Upper right, far back over the wall: a small locomotive trailing a plume of smoke under a hard blue sky', detail: 'Over the wall, on the horizon, a small locomotive pulls a line of smoke across a bright sky. The railway recurs all through de Chirico’s early work and is usually read as a nod to his father, Evaristo, a railway engineer who built lines in Greece, where the artist was born. Treat the autobiography as a common reading rather than a certain fact; what is plainly there is a modern, moving train pulled into a still dream.' },
...
function SngMetaphysical({ accent, onZoom }: { accent: string; onZoom: (src: string, cap: string) => void }) {
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseStyle}>
        He was after one effect above all, and he had a word for it: the <strong>enigma</strong>. The sense that an ordinary square, at a strange hour, is concealing something it will not tell you. (He liked the idea so much that he once inscribed a 1911 self-portrait with a Latin motto, &ldquo;<em>Et quid amabo nisi quod aenigma est?</em>&rdquo; &mdash; &ldquo;And what shall I love if not the enigma?&rdquo; That line belongs to the self-portrait, not to the painting we are about to look at, but it is the credo behind all of it.) The mood owes a great deal to the German philosopher <strong>Friedrich Nietzsche</strong>, whose vision of empty, ominous afternoons saturates de Chirico&rsquo;s early work.
      </p>
      <p style={proseStyle}>
        One thing has to be said plainly before we go further, because it is the most-repeated error about this picture. <strong>De Chirico was not a Surrealist.</strong> The Surrealist movement, the one Andr&eacute; Breton would launch with a manifesto, did not exist in 1914; it was founded a full <strong>decade later, in 1924</strong>. De Chirico is its great <strong>forerunner</strong>, the painter the Surrealists would later worship as a prophet of their own program, but he came before them and was never one of them (in fact, by the time the movement formed he had largely turned against the modern avant-garde and they fell out). The picture in front of us is a 1914 Metaphysical painting that fed a movement which did not yet exist.
      </p>
    </article>
  )
}
...
function SngMaking({ accent, onZoom }: { accent: string; onZoom: (src: string, cap: string) => void }) {
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseStyle}>
        The objects are three. First, a <strong>plaster head of a classical statue</strong>, white and serene, the face of an idealized ancient god. Second, an oversized <strong>rubber surgeon&rsquo;s glove</strong>, limp and drooping, the kind a doctor pulls on, tacked flat to the wall beside the head. Third, on the ground below, a single <strong>green ball</strong>. There is no story that connects a god&rsquo;s head, a surgical glove, and a child&rsquo;s ball. They are bound together only the way a dream binds things, by sitting in the same frame and refusing to explain themselves.
      </p>
      <p style={proseStyle}>
  ...
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseStyle}>
        ...
        he first surprise is the size. Reproductions make <em>The Song of Love</em> feel monumental, but the real panel is small: about <strong>two feet five inches tall by under two feet wide</strong> (73 by 59 centimetres), upright, smaller than a kitchen tray stood on end. Everything in it is rendered with a flat, deadpan, almost diagrammatic clarity, and that clarity is the trap. The strangeness does not come from any distortion (nothing is smeared, fractured, or melted) but from how calmly impossible things are laid out as plain fact.
      </p>
      <p style={proseStyle}>
      ...
      <p style={proseStyle}>
        Nothing in the square connects by ordinary logic. The head does not belong with the glove; the glove does not belong with the ball; none of them belong with the distant train. They cohere only as a dream coheres, and the picture is serene on its surface and uneasy all the way down. The Greeks had a word the painter loved for this feeling, and the painting itself supplies the English one: a <strong>song of love</strong> sung in a language no one in the square can hear.
      </p>
    </article>
  )
}
  ...
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseMutedStyle}>
        (A note on the wording, since translations vary. The line comes from the French and is sometimes rendered &ldquo;my eyes saw <em>the mind</em> for the first time,&rdquo; from the same word, <em>la pens&eacute;e</em>. &ldquo;Thought&rdquo; is the standard English. And it bears repeating that Magritte saw a reproduction shown to him by Lecomte, not the original painting.)
      </p>
      <p style={proseStyle}>
