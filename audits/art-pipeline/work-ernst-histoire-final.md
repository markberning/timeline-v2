# FINAL — Max Ernst, *Histoire Naturelle* (Natural History) (1926)

Reconciled work-read for the Surrealism chain. All [BLOCKER]/[FIX] items from the
three gate files folded in. FACT gate: clean PASS, hero credit confirmed exactly
(plate 30, *The Fugitive / L'Évadé*, NGV Melbourne) — no softening needed. READ gate:
meta-narration / forward-reference / reader-command cuts applied; "frottage" stays
inline-defined on first use; JSX em-dashes standardized to entities. FRAME gate:
1924 Surrealism-founding anchor added, automatism "cousin" mechanism closed, the
"chance as a legitimate engine" claim scoped against Dada's prior chance use,
"Frottage is born" qualified to "as a deliberate method," décalcomania dated/attributed.

Two parts: PART A is the `ERNST_HISTOIRE` const for `src/lib/art-content.ts`; PART B
is the five `Ern*` section components + the NARRATIVES registry comment for
`art-section-reader.tsx`. Identifiers / section-ids / component names unchanged.
