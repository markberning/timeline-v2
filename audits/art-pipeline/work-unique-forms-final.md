# FINAL — Umberto Boccioni, *Unique Forms of Continuity in Space* (1913)

Resolved through the gated Art content pipeline (fact pack → Opus author → 5 critic gates → resolver/reviser). This is the FINAL output, with all gate FIXes folded in. PART A is the `ArtWorkContent` const for `src/lib/art-content.ts`; PART B is the five `Unq`-prefixed section components for `art-section-reader.tsx`.
...
**FACT LEDGER (verified at author + resolver time):**
- Original 1913 is **plaster** (now MAC USP, São Paulo); "wax" is a disputed minority claim — used plaster. [DOCUMENTED]
- **No bronze cast in Boccioni's lifetime.** ALL bronzes are posthumous: first two in 1931 (MoMA's cast dated by MoMA "1913, cast 1931 or 1934"); later mid-century casts and a 1972 group; some 1972 casts are casts-of-an-existing-bronze (cast-of-a-cast). Never imply Boccioni cast the bronze. [DOCUMENTED]
- **Met cast** confirmed against the LIVE Met collection API (object 485540): accession **1990.38.3**, objectDate **"1913, cast 1950"**, accessionYear 1990, creditLine **"Bequest of Lydia Winston Malbin, 1989"**, 47¾ × 35 × 15¾ in, 200 lb. The FACT gate independently re-confirmed this against the JSON endpoint. Some secondaries (Wikipedia) date the Met/Milan casts 1949; the Met's own record reads **1950**, so we KEEP 1950 and do not assert a standalone "1949 bronze" the work never otherwise places. [DOCUMENTED — Met collection API]
- Boccioni died **1916, age 33**, of injuries after being thrown from a horse in WWI cavalry training (near Verona). [DOCUMENTED, standard account]
...
- Marinetti (Futurism's founder) later **co-founded Italian Fascism** (co-authored the 1919 Fascist manifesto, early Mussolini ally). Boccioni died **1916**, three years before the Fascist movement existed — so the movement's Fascist afterlife belongs to the movement, NOT to Boccioni or the 1913 work. [DOCUMENTED]
- "Glorify war — the world's only hygiene" is one clause of the 1909 Founding Manifesto's war-glorification line; quoted with its antecedent intact. [DOCUMENTED, verbatim]
...
All five `id`s in PART A match the registry keys in PART B. `medium`, `dimensions`, `rights`, `heroImage`/`heroFit`/`heroAspect`, `stats`, `sections`, `provenance`, `figures`, `annotations` (6), and `lineage` are all populated per the STARRY_NIGHT field set. New prose contains no `—` characters (em dashes are rendered via `&mdash;` entities only, matching the house style); verbatim quotes preserve their original punctuation.

---

### Resolver note — gate FIXes folded in

- **FRAME [BLOCKER] (Met cast year):** RESOLVED upstream — the FACT gate re-confirmed "1913, cast 1950" against the live Met JSON API (object 485540, accession 1990.38.3, Malbin bequest 1989). KEPT 1950 in all places; ledger now documents the 1949-vs-1950 source split.
- **FRAME [FIX-1] Marinetti→Fascism:** ADDED as a new final paragraph in `UnqDeath` and to the Marinetti `figures` role — framed as the movement's afterlife, with explicit anti-anachronism guard ("Boccioni, dead in 1916, had no part in that").
- **FRAME [FIX-2] "world's only hygiene":** tightened to the full verbatim clause "glorify war — the world's only hygiene," antecedent now unambiguous.
- **FRAME [FIX-3] superlatives:** "one of the most widely held images of the twentieth century" → "one of the most widely circulated images of any twentieth-century artwork" (scoped to the coin mechanism); "the most important sculpture of his career" → "the sculpture that would become his masterpiece."
- **FACT [FIX-1] provenance "1949 bronze":** softened to "one of the existing mid-century bronzes (a cast of a cast)"; standalone "1949 bronze" removed.
- **READ [S1] roadmap meta-narration:** cut "We are going to walk straight back into it later," "we are about to look at," and "We will get to how, and why that matters" in `UnqManifesto` (kept the punchy "theory in 1912, plaster in 1913, artist gone by 1916" beat).
- **READ [L1] flange redundancy:** compressed the physical description in `UnqMaking` (dropped the second "air carving it" image) so the guided looking lands fresh in `UnqLooking`; added a light pointer to the next section.
- **READ [C2] define casting:** added the mold-and-pour mechanism once, at first cast in `UnqAfterlife`.
- **READ [S2] / [L2] (NICE):** softened "It is worth sitting with the strangeness… for a moment" to "The strangeness here is real and rarely said plainly"; added one "as you walk around it" sentence honoring the in-the-round opener.
- **READ [S3] (NICE):** varied the two closing "wings" images — hook keeps "wings of flame," section close now "wings of speed."
