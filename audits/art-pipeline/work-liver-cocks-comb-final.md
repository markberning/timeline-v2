# FINAL (reconciled) — The Liver Is the Cock's Comb (Gorky, 1944) — WORK read

Reconciled from `work-liver-cocks-comb-draft.md` against the three gates
(`-gate-fact.md`, `-gate-read.md`, `-gate-frame.md`) and the factpack (SOURCE OF TRUTH).
Every [BLOCKER] (none) and [FIX] folded. PART A = the `LIVER_COCKS_COMB` const for
`src/lib/art-content.ts`. PART B = the five `Lvr*` chapter components + NARRATIVES registry
comment. RIGHTS: `in-copyright` (1944 work, US term ~95 yrs from publication → through ~2040;
shown small under fair use, credited Buffalo AKG + Gorky estate / ARS). KEY STATEMENT =
Breton's 1945 "nature as cryptogram" intro, now closed with a marked ellipsis so no words are
silently dropped. Identifiers / section-ids / component names IDENTICAL to draft.

Fixes folded:
- FACT-1: Breton cryptogram quote no longer silently truncated — trailing ellipsis marks the cut.
- READ: setting title now matches Part A↔Part B; Afterlife eyebrow/dateLabel/prose dates reconciled;
  redundant Surrealism gloss trimmed; honesty-label tics ("the honest answer", "keep this honest")
  scrubbed; reader-command/imperative density thinned; "automatism"/"biomorphic" kept inline-defined.
- FRAME: "died of starvation in his arms" → "died of starvation in 1919" (unsourced dramatization cut);
  heir roster widened (NY School figure added so it doesn't read as de Kooning alone); "the most gifted
  apprentice in America" softened to "one of the most gifted apprentices in America"; bridge claim kept
  scoped as synthesis-and-influence, never sole origin; genocide/lost-mother biography kept proportionate.
