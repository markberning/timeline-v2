# DRAFT — René Magritte, *The Treachery of Images* (Ceci n'est pas une pipe), 1929

Authored through the art content pipeline from `work-treachery-factpack.md` (SOURCE OF
TRUTH). kind = WORK, Modern era, Surrealism movement. Voice modeled on
`absinthe-narratives.tsx.txt`; const shape modeled on `STARRY_NIGHT` / `CITY_RISES`;
manifesto/key-statement handled in-prose (the painted caption + the sourced Magritte
gloss), as `CtyManifestos` handles the Futurist manifesto. Five sections, ids ascending
by `progress`: `proposition` · `making` · `looking` · `break` · `afterlife`. American
spelling. Imperial dimensions only. Reader has ZERO prior knowledge, every term
inline-defined on first use.

**FACT HANDLING per fact pack:**
- The **1929 LACMA oil** (acc. 78.7) is THE canonical work and the wired image. NOT the
  1948/52 *L'air et la chanson* variant, NOT the 1966 *Les deux mystères*. Guarded in
  `TrcAfterlife`.
- Dimensions: LACMA's **60.3 × 81.1 cm = 23 3/4 × 31 15/16 in (≈ 1 ft 11 3/4 in ×
  2 ft 7 15/16 in)**. The stale 63.5 × 93.98 cm figure refused.
- Caption verbatim: **Ceci n'est pas une pipe** ("This is not a pipe"); the painted
  statement is true, that is the point. The image of a pipe is not a pipe.
- Magritte gloss verbatim per Torczyner (1977), p. 71; attributed to Torczyner, NOT
  over-specified as to when/where he said it.
- Foucault: essay **1968** (journal), book **1973**; never "1973" as the only date.
- Provenance: Magritte 1929 → Galerie Le Centaure → Mesens 1932 → Copley 1957 → LACMA
  1978. No prices (the ~$115,000 figure is secondary, refused).

**HOOK (work header):** A pipe painted as plainly as a catalogue illustration, over the
tidy line "this is not a pipe," and the sentence is true: what hangs on the wall is an
image of a pipe, not a pipe, which is the whole argument.
...
// (ART_IMG.magrittePipe) — NOT the 1948/52 L'air et la chanson variant, NOT the
// 1966 Les deux mystères. Authored through the art content pipeline.
  ...
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseStyle}>
        ...
        ook at the painting and the first thing you see is the plainest thing in the world: a <strong>pipe</strong>. A single brown tobacco pipe, bowl on the right, stem on the left, painted with the cool, even competence of an illustration in a catalogue. Then your eye drops to the line written underneath it, in a tidy, almost schoolroom hand, and the line says: <em>&ldquo;Ceci n&rsquo;est pas une pipe.&rdquo;</em> That is French for <strong>&ldquo;This is not a pipe.&rdquo;</strong> A picture of a pipe, captioned, in careful cursive, with the words <em>this is not a pipe.</em>
      </p>
      <p style={proseStyle}>
      ...
      <p style={proseStyle}>
        Here is the painter. <strong>René Magritte</strong> (1898&ndash;1967; pronounced &ldquo;reh-NAY ma-GREET&rdquo;) was a Belgian artist who, by 1929, had settled into the deadpan style that would define his Surrealism. <strong>Surrealism</strong> was the art movement, launched in Paris in 1924, that set out to unsettle ordinary reality, often by dredging up dream-logic and the unconscious mind. Most Surrealists did this with melting, fantastical, hallucinatory images. Magritte did the opposite. He painted ordinary objects with the bland clarity of a department-store ad, and then arranged them into a situation that quietly short-circuits the eye. <em>The Treachery of Images</em> is the purest thing he ever made in that vein, because there is no fantasy in it at all, just a pipe, a blank background, and a true sentence that feels like a lie.
      </p>
      <p style={proseStyle}>
        The work travels under several names, and it helps to sort them. Its formal title is <em>The Treachery of Images</em>, in French <em>La trahison des images</em>. The line painted across the bottom, <em>Ceci n&rsquo;est pas une pipe</em>, is so famous that people often use it as the title too. (A note for later: Magritte returned to this captioned pipe more than once across his career, so there are several pictures floating around with that line in them. The one we are talking about, the original and the canonical one, is the <strong>1929 oil now in Los Angeles.</strong> Hold that, because it matters when we get to the other versions.) Lead with the proper title, treat the painted line as the famous caption, and keep the two separate in your head.
      </p>
      <p style={proseStyle}>
        Because everything in this painting depends on one move you have to feel before you can think about it: the gap between a thing and the picture of a thing. The picture is going to spend the next four sections holding that gap open with both hands. So before we look at how it is painted, sit for a second with the plain fact that started it: this is a true sentence, written under a picture, and the truth of it is the whole point.
      </p>
    </article>
  )
}
...
function TrcMaking({ accent }: { accent: string; onZoom: (src: string, cap: string) => void }) {
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseStyle}>
        Magritte made that gap the subject of a whole series. He would paint an object and then label it with the wrong word, or paint a word where the object should be, or set image and text side by side so they argued. The captioned pipe is the cleanest and most ruthless of them, because it strips the experiment down to one object, one sentence, and a blank field, and then refuses to blink.
      </p>

      <SectionHeader accent={accent} label="Words and Images" title="The same year, the idea written out as a diagram" />
      <p style={proseStyle}>
        The clearest proof of what he was after sits in another document from the very same year. In 1929 Magritte published a short illustrated essay, <em>Les mots et les images</em> (&ldquo;Words and Images&rdquo;), in the Surrealist journal <em>La R&eacute;volution surr&eacute;aliste</em> (&ldquo;The Surrealist Revolution&rdquo;). It is not a painting. It is a page of small diagrams and blunt propositions about exactly this gap, and one of its lines is the key to the whole pipe: <em>an object never performs the same function as its name or its image.</em> That is the captioned pipe in a sentence. The picture and the essay are twins, made in the same year by a man who had decided, very deliberately, to put the thing and the picture and the word in front of you at once and make you watch them fail to line up.
      </p>

      <SectionHeader accent={accent} label="What the painter said" title="“Could you stuff my pipe?”" />
      <p style={proseStyle}>
        And Magritte, who was usually cool and unhelpful when people asked him to explain his pictures, did once explain this one, and it is worth quoting in full because it is the painting talking. The remark is recorded by the collector and writer Harry Torczyner, in his book <em>Magritte: Ideas and Images</em> (1977), and it runs:
      </p>
      <p style={proseStyle}>
  ...
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseStyle}>
        ...
        tart with the size, because it surprises people. The painting is small and wide: about <strong>two feet across and just under two feet tall</strong> (the kind of modest, near-tabletop canvas you could carry under one arm), painted in <strong>oil on canvas</strong>, the standard mix of pigment in linseed oil that European painters had used for centuries. Everything that follows happens inside that small landscape frame, and the modesty is part of the deadpan. This is not a grand statement-sized picture. It is the size of a diagram.
      </p>
      <p style={proseStyle}>
        Now the pipe. In the center of the canvas sits a single <strong>brown briar tobacco pipe</strong>, bowl to the right, stem reaching off to the left. Look at <em>how</em> it is painted, because the handling is the first half of the argument. It is rendered with smooth, even, unglamorous clarity: gentle modeling on the bowl, a soft sheen along the stem, no visible brushwork to speak of, no flourish, no drama. It looks less like a &ldquo;painting&rdquo; of a pipe than like an <strong>illustration</strong> of one, the kind you would find in a smoker&rsquo;s mail-order catalogue or pinned up as a teaching plate in a classroom. That choice is deliberate. The plainer and more matter-of-fact the pipe, the more it reads as a generic <em>example</em> of a pipe, a sign rather than a portrait, which is exactly what Magritte needs it to be.
      </p>

      <SectionHeader accent={accent} label="The ground" title="Floating on nothing" />
      <p style={proseStyle}>
        Look at what the pipe is sitting on, and you will find that it is sitting on nothing. There is <strong>no shadow</strong> under it, no table, no shelf, no surface of any kind. The pipe simply floats in the middle of a flat, neutral, <strong>light tan-brown field</strong> that fills the whole rest of the canvas. No room, no horizon, no window, no context. Magritte has scrubbed away everything that would tell you this is a scene, a place, a moment. What is left is the bare image, hung in a void. That emptiness does quiet, heavy work: it stops you from reading the pipe as &ldquo;a pipe on a table in a room&rdquo; and forces you to read it as what it actually is on the canvas, a depicted object, a sign with nothing around it but blank ground.
      </p>

      <SectionHeader accent={accent} label="The caption" title="A line in schoolroom cursive" />
      <p style={proseStyle}>
        Below the pipe, centered, runs the famous line: <em>Ceci n&rsquo;est pas une pipe.</em> Look at the lettering itself, because the <em>style</em> of the writing matters as much as the words. It is a tidy, even, handwritten-looking <strong>cursive</strong>, the kind of careful schoolroom script a teacher would chalk on a board or a copybook would print at the top of a page for a child to copy out. It is not a painter&rsquo;s flourish and not a printed label. It is the handwriting of a <em>lesson.</em> And that is the trap closing, because the whole painting is built in the shape of an educational chart: an object on top, a label underneath, exactly the layout of every &ldquo;this is a ___&rdquo; teaching plate you ever saw as a child. Magritte borrows that format precisely so he can break its promise. The chart format swears to you that the label below names the thing above. This one refuses. The lesson-plate says, in its neat little hand, that the thing above is <em>not</em> what it so plainly looks like.
      </p>

      <SectionHeader accent={accent} label="The deadpan" title="Calm illustration and flat denial on one surface" />
      <p style={proseStyle}>
        Now pull back and take the whole thing in at once, because the final effect is in the <strong>tone.</strong> Nothing anywhere in the picture signals that a joke is being told. There is no wink, no exaggeration, no surreal melting or distortion, none of the lurid dream-imagery you might expect from the word &ldquo;Surrealism.&rdquo; The pipe is calm. The ground is calm. The lettering is calm. The contradiction (a clear picture of a pipe, captioned <em>this is not a pipe</em>) is delivered with a completely straight face, in the most untroubled paint imaginable. That flatness <em>is</em> the argument. By making the picture so cool and so plain, Magritte makes you notice that a perfectly ordinary illustration and an outright denial of that illustration are sharing one small canvas without any visible strain at all. The two things that should not be able to sit together (the image that says &ldquo;pipe&rdquo; and the sentence that says &ldquo;not a pipe&rdquo;) sit together perfectly calmly, because, once you see it, they were never actually in conflict. The picture was never a pipe. The caption just says so out loud.
      </p>
    </article>
  )
}
