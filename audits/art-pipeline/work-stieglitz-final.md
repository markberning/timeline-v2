# RECONCILED FINAL — Francis Picabia, *Ici, c'est ici Stieglitz, foi et amour* (1915)

Work id: `stieglitz`. Movement: Dada. Reconciled after all three gates (fact / read / frame). Every [BLOCKER] and [FIX] folded; NICE items applied where they sharpen the read. Two parts: PART A is the `ArtWorkContent` const for `src/lib/art-content.ts`; PART B is the five `Sti*` section components for `art-section-reader.tsx` plus the trailing NARRATIVES registry comment.

Gate reconciliation summary:
- FACT [FIX] — `heroAspect: 0.71` kept (the displayed hero is the image FILE, 1003×1418 ≈ 0.71, under heroFit:'contain'); inline comment corrected to cite the image file, not the artwork's cm dimensions.
- FRAME [BLOCKER] — de Zayas 1913 "absolute caricatures" precedent added; "his own invention" / "the thing he is remembered for" removed (Picabia no longer framed as sole inventor).
- FRAME [FIX] — "something genuinely new in the history of portraiture" scoped to "the human figure thrown out altogether."
- FRAME [FIX] — Dada-timing corrected: the word "Dada" coined at the Cabaret Voltaire in 1916, after this 1915 proto-Dada work; the "erupting at the same moment in Zurich" simultaneity cut.
- READ [FIX] ×5 — reader-commands, meta-narration, and honesty-label framing scrubbed from StiLooking and StiAfterlife (content preserved).
- NICE — companion mechanomorphs named; "most important photographer" / "single-handedly" softened once each; blackletter defined fully once.
