# Work-read FINAL (reconciled) — Jackson Pollock, *Autumn Rhythm (Number 30)* (1950)

Reconciled from `work-autumn-rhythm-draft.md` against all three gates
(fact / read / frame). Every [BLOCKER] and [FIX] folded; no BLOCKERS were
raised. Source of truth: `work-autumn-rhythm-factpack.md`.

**Changes folded in this pass:**
- **FACT [FIX] — palette.** "no blue" was false; verified palette is mostly
  black/white/brown/tan WITH a faint trace of teal/turquoise. Corrected in PART B
  `AutLooking` AND the PART A `skeins` annotation (label updated too).
- **READ [FIX] — meta-narration.** Cut "so we'll come to that in the next chapter"
  (Floor §3) and "this chapter sets… the next four get into the paint" (Floor "Why
  this is a beginning"); trimmed "though, as we'll see" in Looking.
- **READ [FIX] — imperative pile-up + "stand inside it" ×3.** Softened the
  command-stack in Looking ("Resist it." rewritten) and varied two of the three
  near-verbatim "stand inside it" beats (hook / Looking open / `scale` annotation).
- **FRAME [FIX] — Namuth films restored** (two films, incl. color), scoped so the
  color film is NOT implied to show *this* canvas; const `making` blurb + Namuth
  `figures` tag updated to match.
- **FRAME [FIX] — "had no real precedent" softened**; added a prose "where it came
  from" beat (Siqueiros's drip workshop, Navajo sand-painting, Surrealist
  automatism, Cubism, and Janet Sobel credited by Greenberg) so the anti-lone-genius
  framing lives in the PROSE, not just `lineage`.
- **FRAME [FIX] — Krasner weight.** `figures` role tag + one clause in Floor prose
  giving her documented role (more-established painter who pulled him into the
  scene); not expanded into a mini-bio.

Voice/format contract held: no meta-narration/reader-commands/honesty-labels/
condescending glosses; PART A plain-string fields use commas/parens/colons (no
literal "—", no `&mdash;`); PART B JSX uses `&mdash;`/`&ndash;`/`&hellip;`/curly-quote
entities; the Pollock "On the floor…" and "no accident" quotes stay verbatim;
"action painting" = Rosenberg 1952; medium enamel; rights `in-copyright`; ft/in;
heroAspect 1.98, heroFit 'contain'. Identifiers/section-ids/component names
identical to draft.
