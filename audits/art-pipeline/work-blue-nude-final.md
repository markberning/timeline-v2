# FINAL — Henri Matisse, *Blue Nude (Souvenir de Biskra)*, 1907

Resolver/reviser output for the gated art pipeline. PART A = paste-ready const for
`src/lib/art-content.ts`. PART B = the five `Blu`-prefixed chapter components for
`art-section-reader.tsx`. The three gates (fact / read / frame) are folded in:

- **FRAME FIX-1** (heaviest surface): a plain, period-aware paragraph now states the
  orientalist charge in the running prose of chapter 1 (French male painter takes a
  colonized place home as an available female "souvenir"), attributed as a live
  scholarly reading, with the critique-vs-reinforce split, and no 21st-century
  moralizing.
- **FRAME FIX-2**: chapter 2's bare "and it is true" softened so it no longer vouches
  past the documented core (the break mechanism is disputed two sentences later).
- **READ fixes**: "primitivism" glossed once where the African-sculpture pressure first
  lands; "modeling" glossed once in the sculpture chapter; about half the signposting
  tics cut; "the scandal that had made him famous" given a three-word gloss; a
  direct-look beat at the aggressive brushwork added to the looking chapter.
- **FACT fix**: "around the show's last day, April 16, 1913" → April 16 stated firmly as
  the Chicago closing day. The copies-burned/effigy-attempted-and-halted/original-never-
  harmed handling and the paraphrased Vauxcelles jeer are preserved verbatim.

Born-verified only; new prose uses parentheses/commas, never em-dashes; imperial
dimensions only.
...
modeling shadow, the oasis, and the loaded brushwork.
