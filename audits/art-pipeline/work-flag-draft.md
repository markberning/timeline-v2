# Work draft — Jasper Johns, *Flag* (1954–55) · id `flag`

Authored against `audits/art-pipeline/work-flag-factpack.md` (source of truth).
Shape mirrors `STARRY_NIGHT`; component pattern mirrors `Cty*` (The City Rises);
voice mirrors the absinthe narratives. Five `Flg*` components, five section ids
(`dream`, `making`, `looking`, `break`, `afterlife`). KEY STATEMENT and "The break"
both render inside the JSX. Rights = `in-copyright` (Johns living, b.1930): image
shown small under fair use.
...
// of method, dated honestly to 1964, not claimed as said about Flag in
// 1954. RIGHTS = in-copyright (Johns b.1930, living): /en/ thumb, shown
// small under fair use, NOT pd-us. No purchase prices in provenance
// (Castelli show → Philip Johnson privately → MoMA gift 1973).
// ─────────────────────────────────────────────────────────────
export const FLAG: ArtWorkContent = {
  ...
  provenance: [
    ...
    { year: '1958–1973', who: 'Philip Johnson (architect)', place: 'New York / New Canaan', note: 'MoMA’s founding director Alfred H. Barr, Jr. wanted Flag for the museum, but trustees worried a painted American flag could read as unpatriotic in the Cold War. Barr’s workaround: the architect Philip Johnson bought it privately, with the understanding it would come to MoMA later. No purchase figure survives in the record.', price: null },
    { year: '1973–today', who: 'Museum of Modern Art', place: 'New York', note: 'In 1973, as Barr retired, Johnson gave Flag to MoMA, “Gift of Philip Johnson in honor of Alfred H. Barr, Jr.” Object 78805. On view.', price: 'gift in honor of Barr', museum: true },
  ],
  figures: [
...
function FlgDream({ accent }: { accent: string; onZoom: (src: string, cap: string) => void }) {
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseStyle}>
        Two small honesties about that quote, because they matter. First, it is genuinely <em>his</em> story, not a critic&rsquo;s invention &mdash; this is Johns describing where the picture came from. Second, the exact phrasing has wandered across his retellings; in a 1960s television interview he gave a looser version (&ldquo;one night I dreamed I painted a flag, and I got up and I painted a flag&rdquo;) and once even teased an interviewer with <em>&ldquo;Which story? Which one do you know?&rdquo;</em> So the dream is real as his account; the precise words are best held loosely. What it tells you is plain enough: he did not labor toward the flag as a clever idea. The most loaded public image in America simply showed up, ready-made, in his sleep, and he went out the next morning and bought what he needed to build it.
      </p>
    </article>
  )
}
...
function FlgBreak({ accent }: { accent: string; onZoom: (src: string, cap: string) => void }) {
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseStyle}>
        One honest caveat, since this is the Pop-Art chapter. <strong>Johns is not, strictly, a Pop artist.</strong> Art historians usually file him under <strong>Neo-Dada</strong>, or call him <strong>proto-Pop</strong> &mdash; a forerunner rather than a card-carrying member like Warhol or Lichtenstein. <em>Flag</em> (1954&ndash;55) actually <em>predates</em> Pop&rsquo;s arrival by several years; it is one of the things that <em>made Pop possible</em>. So the right way to place this picture is as a <strong>bridge</strong>: it leads out of Abstract Expressionism and toward Pop, standing in the doorway between them. That is the break, scoped honestly &mdash; not the first Pop painting, but the one that cleared the ground.
      </p>
    </article>
  )
}
...
function FlgAfterlife({ accent }: { accent: string; onZoom: (src: string, cap: string) => void }) {
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseStyle}>
        Barr found a way around the problem. He persuaded the architect <strong>Philip Johnson</strong> (1906&ndash;2005) to <strong>buy <em>Flag</em> privately</strong>, with the understanding that it would come to MoMA later, once the heat had passed. And that is exactly what happened. In <strong>1973</strong>, as Barr was retiring, Johnson <strong>gave the painting to the museum in Barr&rsquo;s honor</strong> &mdash; which is precisely what the credit line still records: <em>&ldquo;Gift of Philip Johnson in honor of Alfred H. Barr, Jr.&rdquo;</em> No sale price is part of the story; it changed hands by purchase and then by gift, and the figures aren&rsquo;t in the public record. The painting MoMA had been too cautious to buy in 1958 it received, free, fifteen years later.
      </p>
