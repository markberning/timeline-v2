# DRAFT — Pauline Boty, *It's a Man's World I* (with companion *II*)
...
// person); tentative figure IDs (matador, jazz musician, Plains Indian) hedged;
// dates 1964–65 not 1963; cancer = malignant thymoma.
// ─────────────────────────────────────────────────────────────
export const ITS_A_MANS_WORLD: ArtWorkContent = {
  ...
  figures: [
    ...
    { name: 'The Independent Group', role: 'British Pop’s male founders (Hamilton, Paolozzi, Blake, Boshier)', palette: ['#5a6a72', '#2e3a42', '#0e1014'] },
  ...
  // Annotations are on Canvas I (the hero). SOURCED figures only; the matador, the
  // jazz musician and the Plains Indian are estate-flagged TENTATIVE and hedged in
  // the prose. Ali = Cassius Clay are the SAME man (he changed his name in 1964),
  // listed once. Prose pointers only, no coordinates.
  annotations: [
    ...
    { label: 'The thinkers and the stars', where: 'Among the crowd of faces', detail: 'Look for Albert Einstein and Marcel Proust, the intellect, set beside Elvis Presley and the Beatles (John Lennon and Ringo Starr) and the film director Federico Fellini with the actor Marcello Mastroianni, the glamour. These are the men Boty openly admired and desired, and naming them as admiration is part of the honesty of the picture, it is a fan’s love-letter as much as an indictment.' },
    { label: 'Power, violence, and the jet overhead', where: 'The hardware and the political faces among the figures', detail: 'A Boeing B-52 nuclear bomber (with an X-15 aircraft) flies over a grand Italian estate; Lenin stands for revolution; and the frame of John F. Kennedy’s assassination, lifted from the Zapruder home movie, sits in the field. The world’s men, the picture says, also run its wars and its killings. Muhammad Ali, the boxer, appears too (he had just changed his name from Cassius Clay in 1964, the year of the painting, the same man under two names) alongside a classical Greek statue (Hermes with the infant Dionysus), the physical ideal.' },
    ...
    { label: 'The grand facades, and the tentative faces', where: 'The architecture behind the figures, and a handful of uncertain identifications', detail: 'Grand architectural facades back the crowd, the kind of stately European estate that signals inherited power. A few figures the estate itself flags as uncertain, so point at them gently: a matador (probably the bullfighter El Cordobés), a jazz musician (possibly Thelonious Monk, though other names are proposed), and an American Plains Indian in a warbonnet. Describe them by type rather than over-naming them; the estate’s own source key keeps these tentative.' },
  ],
  lineage: {
    parents: [
...
function ManLooking({ accent }: { accent: string; onZoom: (src: string, cap: string) => void }) {
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseStyle}>
        A few figures in the field are genuinely uncertain, and the estate that holds the work flags them as such, so it is worth pointing at them gently rather than over-naming them. There is a <strong>matador</strong> (probably the bullfighter <strong>El Cordobés</strong>); a <strong>jazz musician</strong> (possibly <strong>Thelonious Monk</strong>, though other names have been proposed); and an American <strong>Plains Indian</strong> in a warbonnet. The estate&rsquo;s own source key keeps these tentative, so read them by type, the bullfighter, the jazzman, the warbonneted figure, and leave the exact names open. The named, certain figures, Einstein, Proust, Lenin, Elvis, the Beatles, Ali, Fellini and Mastroianni, the Kennedy frame, the bomber, the Hermes, carry the picture on their own.
      </p>
    </article>
  )
}
