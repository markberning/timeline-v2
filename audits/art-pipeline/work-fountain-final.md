# FINAL — Duchamp, *Fountain* (1917)

Reconciled from `work-fountain-draft.md` against all three gate files (fact / read / frame) and `work-fountain-factpack.md`. Every BLOCKER and FIX folded in. Two blocks below: PART A is the `FOUNTAIN` const; PART B is the five `Fnt*` section components plus the NARRATIVES registry line. Identifiers, section ids, and component names are unchanged from the draft.
