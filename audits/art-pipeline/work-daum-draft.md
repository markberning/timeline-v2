# DRAFT — *"Daum" Marries Her Pedantic Automaton "George" in May 1920, John Heartfield Is Very Glad of It* (Grosz, 1920)

Work-read for the art vertical, id `daum`. Authored from `work-daum-factpack.md` (source of truth). Two blocks below: **PART A** is the `DAUM` const (drop into `src/lib/art-content.ts`, register in `ART_WORK_CONTENT`); **PART B** is the five `Dau*` chapter components + the NARRATIVES registry line (drop into `art-section-reader.tsx`).

Fact handling locked: medium is **watercolour board / works-on-paper**, NOT canvas (Berlinische Galerie). The "Marshal Pétain" alt-title is **wrong** (that is *Republican Automatons*, MoMA) and is not used. "Daum" is the anagram of "Maud," Grosz's nickname for his wife Eva (surname Peter/Peters unresolved → hedged). The "meta-mechanical construction" tag is presented as Grosz's **descriptor**, not a painted subtitle. KEY STATEMENT uses the Schneede-1975 "despair, hate and disillusionment" line, **labelled as a general statement** about Grosz's art, not about this work; the "gun and a sword" line is described, never quoted as verbatim. Individual collaged scraps are kept general (not reliably identifiable). Dimensions ft/in only.
...
export const DAUM: ArtWorkContent = {
  ...
  heroAspect: 0.71, // 30.2 × 42 cm → W/H ≈ 0.72, PORTRAIT, a small sheet
  ...
  sections: [
    { id: 'berlin', eyebrow: 'Berlin · 1920', dateLabel: 'May 1920', title: 'A wedding announcement, drawn in acid', blurb: 'Grosz comes home from the war to a Berlin of revolution, hyperinflation, and amputees, joins Berlin Dada, and marries Eva Peter the same month he makes this picture about the marriage.', progress: 0.08 },
    ...
    { id: 'afterlife', eyebrow: 'After', dateLabel: '1922–today', title: 'From a Malik portfolio to a Berlin museum', blurb: 'Grosz reproduced it in his 1922 portfolio as a meta-mechanical construction; it surfaced in the Berlinische Galerie in 1995, and now hangs as one of the sharpest things Berlin Dada made.', progress: 0.96 },
  ],
  provenance: [
  ...
  annotations: [
    ...
    { label: 'He is literally collaged', where: 'The groom’s body, set against the hand-painted bride', detail: 'Where the bride is drawn and painted by hand, the husband is assembled from photographs cut out of newspapers and magazines and pasted down (collage, also called photomontage when the scraps are press photographs). The two halves of the picture use two opposed techniques on purpose: living, painted flesh on one side, cut-and-pasted machine-man on the other. The individual clippings are not reliably identifiable, so what matters is the method, not which photo is which.' },
    { label: '“Daum,” the bride', where: 'Beside the groom, a pale woman in underwear and a floppy grey hat', detail: 'The bride is rendered in soft, traditional ink and watercolour: largely undressed, breast and genitals exposed, sensual, turned toward the machine-man who will not turn toward her. “Daum” is an anagram of “Maud,” Grosz’s nickname for his wife, Eva (her surname is given variously as Peter or Peters, so we leave it open). Vivid painted flesh against cold pasted mechanism, the whole tension of the picture in one pairing.' },
  ...
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseStyle}>
        ...
        irst, scale. For all its fame this is a <strong>small sheet</strong>, about <strong>one foot four and a half inches tall by just under a foot wide</strong> (roughly 42 by 30 centimeters), <strong>portrait-shaped</strong>, taller than it is wide. It is not a wall-sized canvas. In fact it is not a canvas at all: it is a work on paper, made in <strong>watercolour, pen and ink, and collage on watercolour board</strong>. (You will sometimes see it described as &ldquo;on canvas&rdquo;; the owning museum is clear that it is a drawing-and-collage on board, and the museum is the authority.) That matters, because the whole picture turns on a contrast between two techniques, and you can see both on the surface.
      </p>
      <p style={proseStyle}>
      ...
      <p style={proseStyle}>
        Look at the groom, right of centre, the smaller of the two figures. He is the <strong>&ldquo;pedantic automaton &lsquo;George&rsquo;&rdquo;</strong> of the title, and he is Grosz&rsquo;s self-portrait: a man assembled from body parts and machine parts, his head a <strong>whitish-grey mask</strong>, gears and fittings marking him as a wind-up mechanism. Note where his attention is. It is <strong>not</strong> on his bride. The whole drama of the picture is in that turning-away: a husband at his own wedding who will not look at the woman beside him.
      </p>
      <p style={proseStyle}>
      ...
      <p style={proseStyle}>
        One caution while you look, because it is easy to over-read a collage. The groom is built from photographs cut out of newspapers and magazines, but the <strong>individual clippings cannot be reliably identified</strong>, which paper, which photo, which machine. Sources do not pin them down, and neither will we. The technique is the meaning here, not the inventory: a body made of pasted press-images, set against a bride made of paint. What the scraps add up to is a man assembled from the mass-media stream, not a man at all.
      </p>
    </article>
  )
}
  ...
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseStyle}>
        ...
        ow the bride. &ldquo;Daum&rdquo; stands beside the machine-man, painted soft and pale in ink and watercolour: <strong>largely undressed</strong>, in underwear and a floppy grey hat, breast and genitals exposed, sensual, turned toward her husband. She is all living flesh and present desire. And the husband, at the very moment she offers herself, <strong>turns away to do arithmetic</strong>. Numbers, figures, calculations cluster around the automaton; he attends, as an early account by the publisher Wieland Herzfelde put it, to &ldquo;soberly pedantic arithmetical problems.&rdquo; That is the joke, told without a punchline: a wedding night in which the groom does sums.
      </p>
      <p style={proseStyle}>
      ...
      <SectionHeader accent={accent} label="A note on the tag" title="A meta-mechanical construction" />
      <p style={proseStyle}>
        One label worth getting right. Grosz called this kind of picture a <strong>&ldquo;meta-mechanical construction&rdquo;</strong> (in German, <em>metamechanische Konstruktion</em>), and he reused that genre-tag when he reproduced the work in 1922. It is his own description of the type of object, the man-machine assembly, and not a second line painted onto the picture. So read it as Grosz&rsquo;s name for what he was making, a constructed machine-man, rather than as part of the inscribed title.
      </p>
    </article>
  )
}
...
function DauAfterlife({ accent }: { accent: string; onZoom: (src: string, cap: string) => void }) {
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <SectionHeader accent={accent} label="After" title="The sharpest small thing Berlin Dada made" />
      <p style={proseStyle}>
        The collaborator named in the title, <strong>John Heartfield</strong>, went on to turn the cut-and-paste technique he and Grosz pioneered into the most ferocious <strong>anti-Nazi photomontages</strong> of the 1930s, the line of political collage that runs straight out of pictures like this one. Grosz himself, his name on a Nazi blacklist, left Germany for the United States in 1933, just before Hitler took power, and lived out most of his career in New York before dying in Berlin in 1959.
      </p>
      <p style={proseStyle}>
        The wedding picture has aged into one of the defining images of <strong>Berlin Dada</strong>: a small sheet, barely a foot and a half tall, that holds the whole bitterness of the movement in a single domestic joke. A man marries a woman and turns away from her to do sums; the husband is a machine and the artist is the husband. It is Grosz&rsquo;s despair, hate and disillusionment, the creed from the start of this read, pointed for once not at the generals or the profiteers but at himself, at his own wedding, in the month it happened.
      </p>
    </article>
  )
}
