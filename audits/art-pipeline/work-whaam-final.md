# Work FINAL — Roy Lichtenstein, *Whaam!* (1963)

Reconciled + revised after all five gates (fact PASS clean; read PASS-with-fixes; frame PASS-with-fixes). All [FIX] items folded. PART A = the `WHAAM` const for `src/lib/art-content.ts`. PART B = the five `Wham*` chapter components for `art-section-reader.tsx`, with the trailing NARRATIVES registry comment. Identifiers, section ids (`source`, `making`, `looking`, `break`, `afterlife`), and component names are identical to the draft.

Changes folded:
- **Age reconcile (fact NICE-1 / read FIX):** const `sections[0].blurb` "forty-year-old" → "thirty-seven-year-old," matching the 1961-switch framing in the prose.
- **Read FIX (reader-command):** cut "so don't let anyone tell you firmly which it is" in `WhamLooking`; stated flatly.
- **Frame C1 (composite-source / fairness):** one clause in `WhamSource` flags the composite-source hypothesis (Grandenetti #90 attacking plane, Heath "Aces Wild" #89 target plane) as scholarly, not fact; the afterlife "also included" line reworded so Heath/Grandenetti read as possible hands in *this* picture, not a career-wide aside.
- **Frame F1 (overclaim/proportion):** the "most recognized image in all of modern art" drumbeat trimmed to one scoped statement; provenance note shortened to "on view"; afterlife prose scoped to Pop / *Drowning Girl*.
- **Frame F3 (NICE, verified, folded):** the ongoing Tate omission of Novick's name added to the credit chapter, framed as "has been noted" (secondary).
- **Frame C2 (NICE, folded):** one prose clause naming downstream influence (appropriation art) in the afterlife close.
- Em-dash / entity audit: no literal "—" in any PART A string; PART B entities sit in JSX children only. Verbatim quote punctuation preserved.
