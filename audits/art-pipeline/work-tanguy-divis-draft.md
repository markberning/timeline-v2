# Work draft — Yves Tanguy, *Indefinite Divisibility (Divisibilité indéfinie)* (1942) · `tanguy-divis`

Authored from `work-tanguy-divis-factpack.md` (source of truth). Surrealism work-read, chain index 9 of 9 (the last). Template twin = `MASSON_AUTO` / `work-masson-auto-final.md` (in-copyright Surrealism work, 5-section shape with a dedicated **break** chapter and the **key statement** inside Looking).

Fact handling locked in:
- **Rights `in-copyright`** (1942 work, Tanguy d. 1955; © Estate of Yves Tanguy / ARS, New York). Hero shown small under fair use; credit carries the rights line. NOT pd-us.
- **Title** = *Divisibilité indéfinie* / *Indefinite Divisibility* (the museum's own pairing; not "Infinite/Endless Divisibility").
- **Invented forms stay invented / non-referential** per Tanguy and the AKG ("without employing direct references to the real world"). "machine," "marble-run," "totem" are third-party ANALOGIES (Artchive) phrased as "reads like," never "is."
- **Dimensions** ft/in only: 40 × 35 in (101.6 × 88.9 cm) → **3 ft 4 in × 2 ft 11 in**, portrait. heroAspect 0.85 (88.9 / 101.6 ≈ 0.875, W/H of the upright canvas ≈ 0.875; brief specifies 0.85).
- **Key statement** = Tanguy's "the painting develops before my eyes" idea. Factpack flags the original publication can't be pinned, so it is attributed to Tanguy and treated as a *documented statement of method* with the source-uncertainty named in prose — no specific publication/date asserted.
- **Institution** = Buffalo AKG Art Museum (formerly Albright-Knox; Albright Art Gallery at the 1945 acquisition) — one museum, three names.
- **Provenance**: Pierre Matisse Gallery, New York → sold to the Albright Art Gallery (now Buffalo AKG) June 22, 1945, via the Room of Contemporary Art Fund; acc. RCA1945:2. No prices.
- Voice/format contract: no literal "—" in plain TS string fields (commas/parens/colons); JSX uses `&mdash;`; `&ldquo;&rdquo;&rsquo;` in JSX; every `<SectionHeader>` has a `label`, first per component `first`; `Tan` prefix; NARRATIVES keys = section ids.
...
export const TANGUY_DIVIS: ArtWorkContent = {
  ...
  annotations: [
    ...
    { label: 'The illusionistic finish itself', where: 'Everywhere; the smooth, hard, modeled surface of every form', detail: 'Step back from the subject and notice the technique. The forms are rendered with smooth, hard, almost academic modeling, the gradual shading that makes a painted object look like a rounded solid you could pick up. The precision is the whole point of the picture. It is what makes a scene that exists nowhere read as an observed, documented place, and it is the thing that separates Tanguy from the loose, smeared, automatic wing of Surrealism.' },
  ],
  lineage: {
    parents: [
  ...
  return (
    <article style={{ padding: '18px 18px 40px' }}>
      ...
      <p style={proseStyle}>
        ...
        ves Tanguy (1900&ndash;1955) built a single world and spent his life painting it. It is always the same place: a vast, deserted plain that recedes to a high horizon, scattered with smooth, pale objects that look like bone or stone or polished metal and resemble nothing that has ever existed. He had no art school behind him &mdash; he was entirely <strong>self-taught</strong> &mdash; and his turn to painting was less a decision than a jolt. Around <strong>1923</strong>, riding past a Paris gallery, he saw a painting by <strong>Giorgio de Chirico</strong> (1888&ndash;1978), the Italian painter of empty, shadow-raked dream-squares, in the shop window, and got off to look at it. By his own account he resolved on the spot to become a painter, despite never having trained as one. The deep, theatrical shadow and the haunted emptiness of de Chirico stayed in his work for good.
      </p>
      <p style={proseStyle}>
