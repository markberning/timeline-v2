# FINAL — Umberto Boccioni, *The City Rises* (La città che sale), 1910

Resolved/revised from `work-city-rises-draft.md` by folding all three gates (fact, read, frame) into the draft. Voice modeled on `absinthe-narratives.tsx.txt`. Born-verified facts only; date is 1910 (1911 = the *Il lavoro* exhibition); "first major Futurist painting" kept Futurism-scoped; no invented prices; the horse-death stated plainly, framed as irony not prophecy. NEW prose uses no em-dash character (parentheses/commas; `&mdash;` entity reserved for verbatim quotes per the JSX corpus). Verbatim-quote punctuation preserved.

**Gate fold-in log:**
- **FACT B1** — death date fixed: thrown 16 Aug 1916, died 17 Aug 1916, age 33, Verona military hospital (`CtyAfterlife`).
- **FRAME F1 (BLOCKER)** — Futurism's politics named honestly in `CtyManifestos` (manifesto's "war &mdash; the world's only hygiene," militarism/violence, "scorn for woman"), plus an anti-anachronism close (Marinetti &rarr; 1919 Fascist Manifesto; Boccioni dead 1916, before Fascism existed). Built on the gate's paste-ready text.
- **FRAME F2** — "the whole reason the painting works" &rarr; "the engine" (`CtySite`); annotation/`CtyLooking` "whole drama" softened to reconcile with the "technique is the hinge" point.
- **READ CLR-1** — "lines of force" reconnected on the canvas in `CtyLooking`.
- **READ CLR-2** — "centrifugal" dropped (Part A annotation + `CtyLooking`), replaced by the plain "inward whirl."
- **READ LOOK-1** — `CtyLooking` plants a forward-hook that the smear is built from discrete dabs.
- **READ STORY-1** — horse-death close de-meta-coached.
- **READ STORY-2** — `CtySite` throat-clear cut.
