# WORK-READ FINAL — Leonora Carrington, *Self-Portrait (Inn of the Dawn Horse)* (c.1937–38)

Reconciled + revised through the art content pipeline (work id `carrington-self`).
Source of truth: `audits/art-pipeline/work-carrington-self-factpack.md` + the three
gates (fact, read, frame). NOT in src/, not committed.

All [BLOCKER] / [FIX] folded:
- **FACT [FIX]** — dimensions height matched to the Met's exact figure: 25 9/16 in =
  **"2 ft 1 9/16 in"** (was "2 ft 1 5/8 in"); width "2 ft 8 in" unchanged. Applied to
  the const `dimensions`, the stat chip, and the `looking` section dateLabel.
- **READ [FIX]** — honesty-label / process-language meta removed from Arrival ¶2 and
  the Afterlife key-statement note (facts kept, self-narration cut); the age-20
  refrain varied; minor editorial/meta tics softened. **Hair stays "dark"** (the READ
  gate's red-mane suggestion is overridden: the FACT + FRAME gates web-verified that
  the canvas shows dark hair; the fact pack's "red mane" is an unsourced overreach).
- **FRAME [FIX]** — one soft "second life / touchstone" sentence added to `afterlife`
  (influence stated in prose, not only in lineage chips); Carrington's figure-role
  tightened to foreground her authorship against Ernst's label.

Voice/format contract observed: no meta-narration / reader-commands / honesty-labels /
condescending glosses; no literal `—` in rendered strings (plain TS fields use commas/
parens/colons; JSX text uses `&mdash;`); the Chadwick muse quote stays verbatim; date
in prose stays **c.1937-38**; `rights:'in-copyright'` with fair-use heroCredit;
dimensions ft/in; `heroAspect:1.25`, `heroFit:'contain'`. Identifiers, section ids, and
