# FINAL — Raoul Hausmann, *Mechanical Head (The Spirit of Our Time)*

Reconciled + revised after the three gates (fact / read / frame). Every [BLOCKER] and [FIX] folded; confirmed-priority items applied verbatim where the gate supplied exact text. Two fenced blocks only: PART A (full `MECHANICAL_HEAD` const) + PART B (5 `Mch*` components + NARRATIVES comment). Identifiers identical to draft (ids berlin/building/looking/break/afterlife). Do NOT let a later pass "fix" `year: 1920` into a range — the integer is the sort field; all PROSE stays "c.1919–20".
